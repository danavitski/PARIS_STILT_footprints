...
! CHG(10/02/03) add ramsflag, need for 16 bit...
      SUBROUTINE PAKINP(RAMSFLG,RVAR,CVAR,NX,NY,NXY,NX1,NY1,LX,LY,      &
     &                  PREC,NEXP,VAR1,KSUM)
...
! CHG (10/01/03) change from 8 to 16 bit
      IF(.NOT.RAMSFLG)SCALE=2.0**(7-NEXP)
      IF(RAMSFLG)SCALE=2.0**(15-NEXP)
...
! CHG (10/01/03) change from 8 to 16 bit, need 2 bytes
         IF(.NOT.RAMSFLG)K=(J-1)*NX+1
         IF(RAMSFLG)K=(J*2-2)*NX+2
...
!        unpack I from element 1 to I sub-grid max
         DO I=2,(NX1+LX-1)
! CHG (10/01/03) change from 8 to 16 bit, need 2 bytes
            IF(.NOT.RAMSFLG)K=(J-1)*NX+I
            IF(RAMSFLG)K=(J*2-2)*NX+I*2
! JCL:(4/24/02) to deal with cases in which ICHAR returns negative integers
!           RNEW=((ICHAR(CVAR(K))-127.0)/SCALE)+ROLD
! CHG (10/01/03) change from 8 to 16 bit
            IF(.NOT.RAMSFLG)THEN
...
! CHG (10/01/03) change from 8 to 16 bit
            IF(.NOT.RAMSFLG)THEN
              ...
              IF(ITMP.LT.0)ITMP=ITMP+256
            ELSE
              ITMP=ICHAR(CVAR(2*K))*256+ICHAR(CVAR(2*K-1))
              IF(ITMP.LT.0)ITMP=ITMP+65536
            END IF
            KSUM=KSUM+ITMP
!           if sum carries over the eighth bit add one
! CHG (10/01/03) change from 8 to 16 bit
            IF(.NOT.RAMSFLG.AND.KSUM.GE.256)KSUM=KSUM-255
            IF(RAMSFLG.AND.KSUM.GE.65536)KSUM=KSUM-65535
         END DO
      END IF

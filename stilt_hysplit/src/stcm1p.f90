...
! CHG(03/12/03)
!
! $Id: stcm1p.f90,v 1.7 2008-05-30 16:03:17 tnehrkor Exp $
!
...
!      real*8 stcprm(15)
      real*8 enx,eny,enz,norm,x1a,y1a
      REAL(KIND(1d0)), EXTERNAL :: cgszll
...
!  stcprm->crotate = stcprm -> gridszeq = 1.0;
      call cpolll(grid%gbase, grid%ref_lat, grid%ref_lon, enx, eny, enz)
      norm = dsqrt(enx*enx + eny*eny)
      if (norm .eq. 0.) then
        call cgrnll(grid%gbase,grid%ref_lat,grid%ref_lon,enx,eny,enz)
        norm = dsqrt (enx* enx + eny*eny)
      endif
      enx = enx/norm
      eny = eny/norm
!  stcprm->gridszeq *= gridsz / cgszll(stcprm, xlatrf,xlonrf);
      grid%gbase(15) = grid%gbase(15) * grid%size / cgszll(grid%gbase,grid%ref_lat,grid%ref_lon, grid)
...
!  stcprm -> srotate = -eny * s_or - enx * c_or;
      ...
      call cll2xy(grid%gbase, grid%sync_lat,grid%sync_lon, x1a,y1a, grid%proj)

...
!
! $Id: prfwrf.f90,v 1.7 2008-06-04 17:15:31 tnehrkor Exp $
...
!   INPUT ARGUMENT LIST:
      ...
      real*8, intent(inout) :: ZSFC,P0,T0,Z0  !these may be set in this routine
      real*8, intent(in) :: U0,V0,w0,mu,muu,muv,msfu,msfv,msft
...
!     mu    - real      dry hydrostatic sfc pressure - ptop (WRF "dry mass" variable)
      ...
      REAL*8, intent(in) ::  U(NZ),V(NZ),W(NZ),T(NZ),Q(NZ),eta(nz),PHALF(NZ), ALT1(NZ), ZSG(NL)
...
!     DEN   - real      air density (kg/m3)
      REAL*8,intent(out) ::   CFXUP1_2(nl),       &
     &      CFXUP2_2(nl),CFXDN1_2(nl),DFXUP1_2(nl),         &
     &      DFXUP2_2(nl),EFXUP1_2(nl),EFXUP2_2(nl),         &
     &      DFXDN1_2(nl),EFXDN1_2(nl),TKEN_2(nl)

      real*8 :: dzdt, maxdeep, maxshall
      real*8, save :: epsmax=1.e-5 !threshold: convective mass fluxes > 0
      real*8, save :: epstotm=0.05 !threshold: fractional mass conservation violation of convective mass fluxes
      real*8, dimension(NZ) :: deta_full, deta_half, half_agl

      integer kflag
      real*8 grav, rdry, p2jm
      SAVE KFLAG

      integer :: kl, kz, kk, klw, kzalt, kzw
      real*8 :: zmdl, pbot, tbot, esat, rbot, zlvl, zbot, wbot
      real*8 :: ptop, ttop, tbar, delz, ztop, ubot, vbot, atop, abot, rtop
      real*8 :: utop, vtop, wtop, frac, omega, temp, alttop, altbot, alt_int
      real*8 :: p1013, pref, frac_alt, frac_cnv, totmas1, totmas2
      real*8 :: CFXUP1_bot,       &
     &      CFXUP2_bot,CFXDN1_bot,TKEN_bot,           &
     &      CFXUP1_top,       &
     &      CFXUP2_top,CFXDN1_top,TKEN_top

      real*8 :: hm(nl), hw(nl+1)
...
!=>compute height of ground surface if not otherwise available
      ...
      if (lprint .and. iprint .le. maxprint) &
           & WRITE(45,'(a,i8,a/(a,l10))') 'Entered PRFwrf with icall=',icall, &
...
!     Compute z-sigma mass and w-level heights AGL
      ...
      IF(.NOT.SFLG)THEN
         if (.not. pflg) then
            WRITE(45,*) 'prfwrf: SFLG and PFLG are FALSE, cannot recover'
...
!         estimate height of 1013 hPa using lowest upper level temp
         ...
         WRITE(45,*) "prfwrf: SFLG=FALSE, computed zsfc from P0,T(1):",zsfc,P0,T(1)
      END IF
...
!     starting height (agl) and vertical velocity
      ...
      if (lprint .and. iprint .le. maxprint) then
         WRITE(45,'(a/(a,g15.6))') 'Input and derived sfc variables:','zsfc=',zsfc, &
              & 'p0 =',p0,'t0=',t0,'tv (tbot)=',tbot, &
              & 'rh (rbot)=',rbot,'u0=',u0,'v0=',v0,'zbot=',zbot,'w0=',w0,'zmdl=',zmdl, &
              & 'mu (Pa)=',mu,'msfu=',msfu,'msfv=',msfv,'msft=',msft
         if (fluxflg) WRITE(45,'(a,g15.6)') 'muu=',muu, &
              & 'muv=',muv
         WRITE(45,'(a/a5,10a15)') 'Input profile:', &
...
!           interpolate alt1 from half levels to w-level:
            ...
            if (kzalt .le. 1) then
               ...
               frac_alt=min(dble(1),(zlvl-half_agl(kzalt-1))/(half_agl(kzalt)-half_agl(kzalt-1)))
            endif
...
!     sounding ends but levels remain to be filled (separately check klw)
      IF(KL .LE. NL .or. klw .le. nl+1)THEN
         IF(KFLAG.EQ.0)THEN
            WRITE(30,*)'WARNING prfwrf: data extrapolation because kl or klw < nl(+1): ', &
                 &  kl, klw, nl
! CHG(09/10/03) correct transformation between sigma and agl
!            ZLVL=ZMDL*(1.0-ZSG(KL))
!           adjust for terrain compression
!            ZLVL=ZMDL*ZLVL/(ZMDL-ZSFC)
!            ZLVL=(1.0-ZSG(KL))*(ZMDL-ZSFC)
            if (kl .le. nl) WRITE(30,*)'from level (m): ', &
...
!           diminish magnitude when no data
      ...
      if (lprint .and. iprint .le. maxprint) then
         WRITE(45,'(a/a5,9a15)') 'Output profile:', &
...
! Input:
         if (iprcnv .le. maxprint) then
            WRITE(45,'(a/a5,11a15)') 'Input convective fluxes:', &
            ...
            do kz=2,nz
               ...
               if (maxshall .ge. epsmax) then
                  ...
                  WRITE(45,'(i5,7(15x),4g15.6)') &
                       & kz,cfxup2_1(kz),dfxup2_1(kz),efxup2_1(kz),totmas2
               endif
            enddo
         endif
         if (iprcnv .le. maxprint) then
            WRITE(45,'(a/a5,11a15)') 'Output convective fluxes:', &
         ...
         do kl=2,nl
            if (maxdeep .ge. epsmax) then
               ...
               if (iprcnv .le. maxprint) WRITE(45,'(i5,7g15.6)') &
...
! adjust de/entrainment to agree with convective mass-flux profile:               
                  ...
                  totmas1=cfxup1_2(kl)-abs(cfxdn1_2(kl)) & !subin
                       ...
                       & + dfxup1_2(kl) + dfxdn1_2(kl) & !detrainment (detupd+_detupk+detdo)
                       & + efxup1_2(kl) + efxdn1_2(kl)   !entrainment (entup, entupk, entdo, edtdoj)
                  if (iprcnv .le. maxprint) WRITE(45,'(5x,7g15.6)') &
            ...
            if (maxshall .ge. epsmax) then
               ...
               if (iprcnv .le. maxprint) WRITE(45,'(i5,7(15x),4g15.6)') &
...
! adjust de/entrainment to agree with convective mass-flux profile:               
                  ...
                  totmas2=cfxup2_2(kl) & !subin
                       ...
                       & + dfxup2_2(kl) & !detrainment
                       & + efxup2_2(kl)   !entrainment
                  if (iprcnv .le. maxprint) WRITE(45,'(5x,7(15x),4g15.6)') &

...
!cg(20100804) same for ECMWF fields
  ...
  if (GRID(KG,kt)%MODEL_ID(2:4) .eq. 'GEO') max_offset = max_offset+1
!cg(20140828) same for ECMWF fields
  if (GRID(KG,kt)%MODEL_ID(1:4) .eq. 'GDAS') max_offset = max_offset+1
  
! Note: For LX1 (or LY1) =1, the following check is more stringent than the check in metpos:
!        Here: II >= LX1+2=3 is considered inside the (sub)grid, 
!   in metpos: II >= 2 is considered inside the grid
!       This could theoretically lead to multiple subgrid expansions/data loads, at least
!       until the full grid is loaded in (see next if-statement)
...
! This should actually never happen, except for the discrepancy in the II/JJ-check with metpos (2 vs 3)
! In Hysplit the following is present (using .or. for i/j, meaning near the edge, not just the corner)
! In Stilt this was deactivated (version 1.5), but was re-activated in version 1.6 (and ever since),
! but with .and. for i/j, meaning it almost never happened either (just at the LL corner)
...
!!$     IF(((II .LT. 3 .OR. II .GE. GRID(KG,KT)%NX-max_offset) .OR. &
!!$          & (JJ .LT. 3 .OR. JJ .GE. GRID(KG,KT)%NY-max_offset)) &
!!$        .AND. .NOT. GRID(KG,KT)%GBLDAT) RETURN	
     IF(((II .LT. 2 .OR. II .GE. GRID(KG,KT)%NX-max_offset) .OR. &
          & (JJ .LT. 2 .OR. JJ .GE. GRID(KG,KT)%NY-max_offset)) &
          .AND. GRID(KG,KT)%GBLDAT.EQ."no") &
          write (KF21,*) 'Warning metsub: unexpectedly near edge, kg=',kg,', ii=',ii,', jj=',jj

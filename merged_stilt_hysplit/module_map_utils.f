...
!               PRINT '(A,F10.3)', 'projection at truelat1: ', proj%truelat1
   ...
   SUBROUTINE ijll_ps(i, j, proj, lat, lon)
      ...
      ELSE
         ...
         !jmh: results are in radians
         if (xx.lt.0. .and. abs((abs(xx/SQRT(r2))-1)).lt.1.e-6) then         
           arccos=pi
         elseif (xx.gt.0. .and. abs((abs(xx/SQRT(r2))-1)).lt.1.e-6) then
           arccos=0.
         elseif (sqrt(r2).lt. max(abs(proj%polei),abs(proj%polej))*1.e-6) then
           arccos=0.
         else          
           arccos = ACOS(xx/SQRT(r2))
         endif
!         print*,'    ARCCOS,xx,r2,abs((abs(xx)-abs(SQRT(r2))))=',arccos,xx,r2,abs((abs(xx)-abs(SQRT(r2))))
         !if (isnan(arccos)) then
         !  print*,'WARNING: arccos=NaN will lead to longitude=NaN'
         !endif
         !end jmh
      ...
      !jmh
!      print*,"jmh: module_map_utilts.F: Returning lat/lon",lat,lon,"for i,j",i,j
      !end of jmh

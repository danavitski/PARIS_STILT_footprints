...
subroutine metinp(BACK,KG,KT,KUNIT,KREC,LX1,LY1,NXS,NYS,NZS,MC,KEND,     &
  ...
  character(4)                :: fluxvars(2) = (/'MUU0','MUV0'/), &
       ...
       geoshgts(2) = (/'HGTS','AHGT'/), &
       geosatem(2) = (/'ATEM','ASPH'/)
  logical                     :: haveflux(2),havewrf(6),havedeep(6),haveshall(3)
  logical                     :: ramsflg, ecmflg, sxtnbit, awrfflg, ppertflg, geosflg
  logical                     :: havegh(2), havega(2), badgeos
  real, allocatable           :: ppert(:,:,:), mupert(:,:)
  real                        :: pmin,ibad_pmin, ptol=50. ! allow for up to 5mb pressure increase with height
  logical                     :: bad_pminp
  integer                     :: ig
  integer                     :: ii,jj
  ...
  geosFLG = GRID(KG,kt)%MODEL_ID(2:4) .EQ. 'GEO'
  SXTNBIT = RAMSFLG .OR. GRID(KG,kt)%MODEL_ID(1:3) == 'ECX' .OR. GRID(KG,kt)%MODEL_ID == 'DWRF' &
        .OR. GRID(KG,kt)%MODEL_ID == 'NWRF' .OR. GRID(KG,kt)%MODEL_ID == 'DGEO'
  haveflux = .FALSE.
  havewrf = .FALSE.
  havedeep = .FALSE.
  haveshall = .FALSE.
  ...
  ! NOTE: size of cdata array dictated by input grid size (see above for nxg,nyg)
  !       P input array dimensioned (NXT,NYT,...) in advpnt (maximum of NXS for all kg)
  !       Only the subgrid (lx1:lx1+nxs-1,ly1:ly1+nys-1) is unpacked in PAKINP,
  !       stored in (1:nxs,1:nys) of the P etc arrays
  if (SXTNBIT) then
  ...
  inloop: DO 
     ...
     READ(KUNIT,REC=JREC,ERR=910)LABEL(1:k50),HEADER(1:k108)
     ...
     READ(LABEL,'(5I2)')IY,IM,ID,IH,IFHR
     
     CALL TM2MIN(IY,IM,ID,IH,0,MC)

     IF((DREC(KG,KT)%TYPE).EQ.2)THEN
        READ(HEADER(1:k108),fmt)IFHR,LENH
     ...
     zloop : DO KK=0,NZS
        ...
        varloop : DO NN=1,NVAR
           ...
           READ(KUNIT,REC=JREC,ERR=920)LABEL(1:k50),CDATA
           if (k50 .eq. 50) then
              READ(LABEL,'(6I2,2X,A4,I4,2E14.7)') IY,IM,ID,IH,IC,LL,VARB,NEXP,PREC,VAR1
           else
              READ(LABEL,'(5I2,1x,i3,2X,A4,I4,2E14.7)') IY,IM,ID,IH,IC,LL,VARB,NEXP,PREC,VAR1
           end if
           ...
           IF(LL.GT.0)THEN
              ...
              CASE ('HGTS') 
                 if (geosflg) then
                    ! Store model level heights in E array:
                    CALL PAKINP(SXTNBIT,E(:,:,LL),CDATA,NXG,NYG,LX1,LY1,NXS,NYS,PREC,NEXP,VAR1,KSUM)
                 else
              ...
              CASE ('AHGT') 
                 ! Store time-averaged height in X array:
                 if (geosflg) &
                      CALL PAKINP(SXTNBIT,X(:,:,LL),CDATA,NXG,NYG,LX1,LY1,NXS,NYS,PREC,NEXP,VAR1,KSUM)
              CASE ('PRES') 
                 ...
                 CALL PAKINP(SXTNBIT,P(:,:,LL),CDATA,NXG,NYG,LX1,LY1,NXS,NYS,PREC,NEXP,VAR1,KSUM)
              CASE ('APRE') 
                 ! Store time-averaged pressure in H array:
                 if (geosflg) &
              ...
              elseif (geosflg) then
                 where (VARB .EQ. geoshgts) havegh = .TRUE.
                 where (VARB .EQ. geosatem) havega = .TRUE.
              endif
           ...
           ELSE
              ...
              if (varb == 'APRS') then
                 ! Store time-averaged psfc in mu:
                 if (geosflg) &
     ...
     IF (ppertflg) THEN
        p = p + ppert
        ! Check for bad pressures: negative total pressure or pressure increasing with height
        ibad_pmin = 0
        DO ll=1,nzs
           ! important: only perform checks on active subgrid
           pmin = minval(p(1:nxs,1:nys,ll))
           bad_pminp = .FALSE.
           if (ll .lt. nzs) then
              bad_pminp = any(p(1:nxs,1:nys,ll) < p(1:nxs,1:nys,ll+1))
           end if
           IF (pmin < 0 .OR. bad_pminp) THEN
              if (pmin < 0 .or. &
                   (ll .lt. nzs .and. any(p(1:nxs,1:nys,ll) + ptol < p(1:nxs,1:nys,ll+1)))) &
                   ibad_pmin = ibad_pmin + 1
              WRITE (*,*) 'metinp: suspect pressure at kg, krec, time,ll,pmin : ',&
                   kg, krec, IY,IM,ID,IH,MC,ll,pmin
              write (*,*) ' dim(p),NXS,NYS,lx1,ly1=',size(p,1),size(p,2),NXS,NYS,lx1,ly1
              do jj=1,nys
                 do ii=1,nxs
                    if (p(ii,jj,ll) < 0 ) write (*,*) ' i+lx1-1,j+ly1-1,p:',&
                         ii+lx1-1,jj+ly1-1,p(ii,jj,ll)
                    if (ll .lt. nzs .and. p(ii,jj,ll) < p(ii,jj,ll+1)) &
                         write (*,*) ' i+lx1-1,j+ly1-1,p,p(l+1):',&
                         ii+lx1-1,jj+ly1-1,p(ii,jj,ll),p(ii,jj,ll+1)
                 end do
              end do
           ENDIF
        ENDDO
        IF (ibad_pmin .NE. 0) THEN
           WRITE (*,*) 'metinp: bad pressure (<0 or increasing with height by more than ptol=',ptol,')'
           DO ll=1,nzs
              write (*,'(a,i4,a,2g15.6)') 'll=',ll,' Min,max ptot=',minval(p(1:nxs,1:nys,ll)),maxval(p(1:nxs,1:nys,ll))
              IF (ppertflg) write (*,'(a,i4,a,2g15.6) ') &
                   'll=',ll,' Min,max ppert=',minval(ppert(1:nxs,1:nys,ll)),maxval(ppert(1:nxs,1:nys,ll))
           ENDDO
     ...
     IF (awrfflg) THEN
        ...
        mu = mu + mupert
     elseif(geosflg) then
        ! For non-awrf/brams inputs, fluxflg denotes time-averaged winds (.TRUE. only for GEOS)
        fluxflg=.TRUE.
        if (.not. all(havegh)) then
           ! geos heights not available, then need the geos average temp/sphu:
           badgeos = any(havegh) .or. .not. all(havega)
        else
           badgeos = any(havega)
        endif
        if (badgeos) then
           WRITE (KFJCLMSG,*) 'metinp: For GEOS, have neither just all height nor just all avg temp/sphu: ', &
                ' havegh= ',havegh,' geoshgts= ',geoshgts,' havega= ',havega,' geosatem= ',geosatem
           stop 'metinp: For GEOS, have neither just all height nor just all avg temp/sphu'
        endif
     ENDIF !awrfflg
     IF(ECMFLG) THEN
           cfu2(:,:,:) = 0.

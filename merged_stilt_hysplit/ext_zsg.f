...
950 continue
  ...
  subroutine ind_zsg(zmdl,zsg,nlvl,zt,zx,aa,bb,cc,vcoord)
    !$$$  SUBPROGRAM DOCUMENTATION BLOCK
    ...
    if (vcoord .eq. 1) then
       ...
       ! general vertical coordinates, use only zt and zsg to find index
       ! Assume zsg are stored bottom up, use the sign_zsg variable to simplify inequality testing:
       sign_zsg=sign(1.,zsg(2)-zsg(1))
       level=nlvl !treat heights above the top level the same as those in the top layer
       do k=1,nlvl
          if (zt*sign_zsg .lt. sign_zsg*zsg(k)) then
             level=k
             exit
          end if
       end do
       if (level .eq. 1) then
          !below lowest level: assumes zsg are layer values, lowest possible index (at ground) is 0.5:
          zx = max(0.5,1.-(zsg(1)-zt)/(zsg(2)-zsg(1)))
       else
          zx = float(level-1) + (zt - zsg(level-1)) / (zsg(level)-zsg(level-1))
       end if
    end if !vcoord
    return
  end subroutine ind_zsg
end module module_ext_zsg

...
!       check if this time period contains a valid grid
        IF(GRID(KG+crr,KT)%NUMBER.LT.0) CYCLE tloop

!       if test grid not equal to current then remap position
        IF(KG+crr.NE.KGC)THEN  

!          for multiple grids convert position from current grid to true
!          then convert true back to new grid coordinates
           IF(GRID(KGC,1)%LATLON.AND.GRID(KG+crr,1)%LATLON)THEN
              CALL GBL2LL(KGC,1,XP,YP,TLAT,TLON)
              CALL GBL2XY(KG+crr,KT,TLAT,TLON,XPT(KG+crr),YPT(KG+crr))
           ELSEIF(     GRID(KGC,1)%LATLON.AND.(.NOT.GRID(KG+crr,1)%LATLON))THEN
              CALL GBL2LL(KGC,1,XP,YP,TLAT,TLON)
              CALL CLL2XY_wps(GRID(KG+crr,KT)%GBASE,TLAT,TLON,XPT(KG+crr),YPT(KG+crr),GRID(KG+crr,KT)%proj)
           ELSEIF(.NOT.GRID(KGC,1)%LATLON.AND.      GRID(KG+crr,1)%LATLON )THEN
              CALL CXY2LL_wps(GRID(KGC,1)%GBASE,XP,YP,TLAT,TLON,GRID(KGC,1)%proj)
              CALL GBL2XY(KG+crr,KT,TLAT,TLON,XPT(KG+crr),YPT(KG+crr))
           ELSEIF(.NOT.GRID(KGC,1)%LATLON.AND.(.NOT.GRID(KG+crr,1)%LATLON))THEN
              CALL CXY2LL_wps(GRID(KGC,1)%GBASE,XP,YP,TLAT,TLON,GRID(KGC,1)%proj)
              CALL CLL2XY_wps(GRID(KG+crr,KT)%GBASE,TLAT,TLON,XPT(KG+crr),YPT(KG+crr),GRID(KG+crr,KT)%proj)
           END IF
...
!tk(20160317)
        IF(GRID(KG+crr,KT)%GBLDAT .EQ. "no")THEN
           max_offset = 2
...
!       (because rows/columns for non-staggered field are padded)
           if (GRID(KG+crr,kt)%MODEL_ID(2:4) .eq. 'WRF') max_offset = max_offset+1
!cg(20100804) same for ECMWF fields
           if (GRID(KG+crr,kt)%MODEL_ID(1:2) .eq. 'EC') max_offset = max_offset+1
           IF(II .LT. 2 .OR. II .GE. GRID(KG+crr,KT)%NX-max_offset .OR. &
              JJ .LT. 2 .OR. JJ .GE. GRID(KG+crr,KT)%NY-max_offset) CYCLE tloop
        ELSE IF(GRID(KG+crr,KT)%GBLDAT .EQ. "nh")THEN
           max_offset = 2
!       For WRF input, stay away from one additional row/column of grid edge
!       (because rows/columns for non-staggered field are padded)
           if (GRID(KG+crr,kt)%MODEL_ID(2:4) .eq. 'WRF') max_offset = max_offset+1
!cg(20100804) same for ECMWF fields
        ...
        ELSE IF(GRID(KG+crr,KT)%GBLDAT .EQ. "sh")THEN
           max_offset = 2
!       For WRF input, stay away from one additional row/column of grid edge
!       (because rows/columns for non-staggered field are padded)
           if (GRID(KG+crr,kt)%MODEL_ID(2:4) .eq. 'WRF') max_offset = max_offset+1
!cg(20100804) same for ECMWF fields
           if (GRID(KG+crr,kt)%MODEL_ID(1:2) .eq. 'EC') max_offset = max_offset+1
           IF( JJ .GE. GRID(KG+crr,KT)%NY-max_offset) CYCLE tloop
        ELSE
           if (II .LE. 0 .OR. JJ .LE. 0) THEN
              CYCLE tloop
           ELSE IF (KG .EQ. NGRD) THEN
              crr=crr-1
           ELSE 
              crr=0
           END IF
...
!       particle must be within temporal domain of one of the files
        ...
        IF(.NOT.BACK)THEN
           IF(JET.GE.FILE(KG+crr,KT)%FIRST%MACC.AND.                &
              JET.LT.FILE(KG+crr,KT)%LAST%MACC)THEN
!             data point falls within the file
              K1=KT
              K2=KT
           ELSEIF(JET.GE.FILE(KG+crr,KT)%LAST%MACC.AND.             &
                  JET.LT.FILE(KG+crr,KT)%LAST%MACC+DREC(KG+crr,KT)%DELTA)THEN
!             data point potentially falls after the end of the file
              IF(K1.EQ.0)K1=KT
              MET2=.TRUE.
           ELSEIF(JET.LT.FILE(KG+crr,KT)%FIRST%MACC.AND.            &
                  JET.GE.FILE(KG+crr,KT)%FIRST%MACC-DREC(KG+crr,KT)%DELTA)THEN
!             data point potentially falls before the start of the file
              IF(K2.EQ.0)K2=KT
              MET1=.TRUE.
           ELSE
              IF(JET.LT.FILE(KG+crr,KT)%FIRST%MACC) MET1=.TRUE.
              IF(JET.GT.FILE(KG+crr,KT)%LAST%MACC ) MET2=.TRUE.
           END IF

        ELSE
           IF(JET.LE.FILE(KG+crr,KT)%LAST%MACC.AND.                 &
              JET.GT.FILE(KG+crr,KT)%FIRST%MACC)THEN
!             data point falls within the file
              K1=KT
              K2=KT
           ELSEIF(JET.LE.FILE(KG+crr,KT)%FIRST%MACC.AND.            &
                  JET.GT.FILE(KG+crr,KT)%FIRST%MACC-DREC(KG+crr,KT)%DELTA)THEN
!             data point potentially falls after the end of the file
              IF(K1.EQ.0)K1=KT
              MET2=.TRUE.
           ELSEIF(JET.GT.FILE(KG+crr,KT)%LAST%MACC.AND.             &
                  JET.LE.FILE(KG+crr,KT)%LAST%MACC+DREC(KG+crr,KT)%DELTA)THEN
!             data point potentially falls before the start of the file
              IF(K2.EQ.0)K2=KT
              MET1=.TRUE.
           ELSE
              IF(JET.LT.FILE(KG+crr,KT)%FIRST%MACC) MET2=.TRUE.
              IF(JET.GT.FILE(KG+crr,KT)%LAST%MACC ) MET1=.TRUE.
           END IF
        END IF

...
SUBROUTINE PUFDSP(KPUFF,UMIX,VMIX,DTM,ZMDL,ZSFC,NLVL,WMIX,ZSG,PAGE,ZPOS, &
                  SIGH,SIGW,HDWP,ZNDX,vcoord)

  use module_ext_zsg, ONLY: ind_zsg
...
! particle in vertical return now
  ...
  if (vcoord .ne. 1) then
     !TBD_sigmap: add support here
     write (*,*) 'ERROR pufdsp: only vcoord=1 supported'
     stop 'ERROR pufdsp: only vcoord=1 supported'
  end if
...
!!$         ZX=(-BB+SQRT(BB*BB-4.0*AA*(CC-ZZ)))/(2.0*AA)
         call ind_zsg(zmdl,zsg,nlvl,sgb,zx,aa,bb,cc,vcoord)
...
!!$         ZX=(-BB+SQRT(BB*BB-4.0*AA*(CC-ZZ)))/(2.0*AA)
         call ind_zsg(zmdl,zsg,nlvl,sgt,zx,aa,bb,cc,vcoord)

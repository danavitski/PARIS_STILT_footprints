...
subroutine PRFCOM(TKERD,TKERN,KG,KT,KSFC,GX,GY,Z0,              &
  ...
  real, allocatable :: cfxup1_1(:),cfxup2_1(:),cfxdn1_1(:),    &
       ...
       tke_1(:)
  logical           :: ramsflg,awrfflg, geosflg
  ...
  INTEGER           :: i,j,k,kret,kzmix,kbls,kk, npar_wrfhyb
  ...
  IF(.NOT.ALLOCATED(PSG))THEN
     ...
     allocate (xv(nlvl),cfxup1_2(nlvl),cfxup2_2(nlvl),cfxdn1_2(nlvl),    &
          ...
          efxup2_2(nlvl),efxdn1_2(nlvl),tke_2(nlvl),alt2(nlvl),stat=kret)
     if(kret.NE.0) then
        write(*,*) '*ERROR* prfcom: memory allocation - diagnostic'
        stop 900
     end if
  ...
  DO J=1,NYS
     DO I=1,NXS
        ...
        datain : DO K=1,NZS
           ...
           if (geosflg) then
              ! Use the TKE/VELV arrays for z, ap, az:
              E1(K) = E(I,J,K)
              H1(K) = H(I,J,K)
              X1(K) = X(I,J,K)
           end if
        ...
        IF( AWRFFLG )THEN
           ...
           npar_wrfhyb=0
           if (allocated(DREC(KG,KT)%wrfvcoords)) then
              npar_wrfhyb=size(DREC(KG,KT)%wrfvcoords,2)
           end if
           ...
           CALL PRFwrf(VMIX,QFLG,UFLG,            &
                ...
                NZS,PSG,P1,U1,V1,W1,T1,Q1,NLVL,ZMDL,ZSG,zsg_full,dzsg_full,              &
                ...
                DFXDN1_2,EFXDN1_2,TKE_2,alt2,vcoord, &
                DREC(KG,KT)%wrfvcoords, DREC(KG,KT)%wrfptop, npar_wrfhyb)
        ...
        ELSEIF( (DREC(KG,KT)%Z_FLAG).EQ.1)THEN
           ...
           if (geosflg) then
              ap0=muu(i,j)
              CALL PRFGEO(VMIX,TKEN,VELV,ZFLG,QFLG,UFLG,TFLG,PRSS,DZDT,       &
                   ZT(I,J),P0(I,J),U0(I,J),V0(I,J),T0(I,J),Z0(I,J),ap0,          &
                   NZS,PSG,P1,U1,V1,W1,T1,Q1,E1,H1,X1,NLVL,ZMDL,ZSG,                 &
                   P2,U2,V2,W2,T2,Z2,Q2,D2,A2)
           else
        ...
        DO J=1,NYS
           DO I=1,NXS
              ...
              elseif (vcoord .eq. 2) then
                 ! couple decoupled velocities at staggered u,v-grid points with mu
                 UMF(i,j,k)=muu(i,j)*u(i,j,k)
                 VMF(i,j,k)=muv(i,j)*v(i,j,k)
                 ! couple decoupled w at staggered w-grid points with mu
                 WMF(i,j,k)=mu(i,j)*w(i,j,k)
              end if
           enddo
        enddo
     enddo
     DO K=0,NLVL-1
        ...
        DO J=1,NYS-1
           DO I=1,NXS-1
              ...
              elseif (vcoord .eq. 2) then
                 ! compute hor. and vert. divergence terms for continuity equation in sigma-p:
                 hdiv=UMF(I+1,J,K+1) - UMF(I,J,K+1) + &
                      VMF(I,J+1,K+1) - VMF(I,J,K+1)
                 DELMASS(I,J,K+1)=(HDIV+VDIV)/mu(i,j)
              end if
           enddo
        enddo
     enddo

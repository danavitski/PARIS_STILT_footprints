...
!
! $Id: output.f,v 1.2 2013/10/22 21:05:52 trn Exp $
!
!$$$
SUBROUTINE OUTPUT (IFILN,IVMAX,VARSIWANT,NTURB,IETIM,KP,ICNDX,XOUT, &
                   ...
                   ZLOCNEXT,PRESLOCAL,TEMPLOCAL,ptyp)

   IMPLICIT NONE

! INTENT(IN) VARIABLES
   INTEGER        , INTENT(IN)  :: IFILN,IVMAX,NTURB,IETIM,KP,ICNDX,ptyp
...
! FILLING THE OUTPUT VECTOR
   DO IV= 1, IVMAX
     ...
     CASE ('zcfx','ZFX1')  !vertical displacement due to
       ...
       WRITE (FORMATO2,'(a) ') '1x,f9.2,'
       WRITING(IV) = ZFXCUM
     CASE ('ptyp','PTYP')  !Pollutant type (integer counter)
       WRITE (FORMATO2,'(a) ') '1x,f8.0,'
       WRITING(IV) = ptyp
     CASE DEFAULT

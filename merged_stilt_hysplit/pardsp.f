...
!                  ypos,zpos,vprime,wratio,uprime,hdwp,zndx,iseed)
SUBROUTINE pardsp(umix,vmix,gdisx,gdisy,dtm,zmdl,zsfc,nlvl,wmix,zsg,zsg_full,dzsg_full,xpos,&
...
!dwen(20090906)                  densnext,tlfrac,xposprev,yposprev,uuprev,uunext,       &
                 ...
                 convdur,zloc,ramsflg,tempnext,tempprev,rhfrnext,        &
                  rhfrprev,sphu,foot,mu1,alt01,mu2,alt02,vcoord)
!************************************************ 
  ...
  real, intent(in)          :: mu1,alt01(:),mu2,alt02(:)
  ...
  real         :: tl(NLVL+1),stdw(NLVL+1),dens(NLVL+1),ubar(NLVL+1),vbar(NLVL+1)
  real :: alt01add(NLVL+1), alt02add(NLVL+1)
  ! JCL:(11/14/02) temporary sigmal levels--needed if add a small 'sublayer' of low turbulence at level of zi
  !      REAL*8 ZSGADD(NZM)
  REAL         :: ZSGADD(NLVL+1)
  REAL         :: zlvls(NLVL), zfull(NLVL)
  real                     :: zposz, dummy
  ...
  !     & ,ZADD(NLVL)
  REAL :: DENSDRY(NLVL),PRS(NLVL),TK(NLVL),E(NLVL),ES(NLVL),R(NLVL),ZADD(NLVL) 
  REAL         :: zlvlsadd(NLVL+1),zfulladd(NLVL+1),zsg_fulladd(NLVL+1),dzsg_fulladd(NLVL+1)
  ...
  DO KTT=1,2
     ...
     IF(KTT.EQ.1)THEN
        !           use VEGHT as fraction of mixed-layer ht if VEGHT <= 9.0; convert VEGHT from [m]=>modified sigma
        IF(VEGHT.LE.9.0)THEN
           VEGHTZ=VEGHT*ZML1
           if (VEGHTZ.gt.ZMDL*0.9) then
              print*,'VEGHTZgtZMDL*0.9:Adjusting VEGHTZ=',VEGHT,' * ',ZML1,' to 0.9*ZMDL=',0.9*ZMDL !jmh 20150806 to prevent index error in ZADD below
              VEGHTZ=ZMDL*0.9
           endif
        ELSE
           VEGHTZ=VEGHT
        END IF
        if (vcoord .eq. 1) then
           VEGHTSIGMA=1.0-VEGHTZ/(ZMDL-ZSFC)
           ZMLSIGMA=1.0-ZML1/(ZMDL-ZSFC)
        elseif (vcoord .eq. 2) then
           call agl2eta(veghtz,veghtsigma,mu1,alt01,zsg,nlvl &
                ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
           call agl2eta(zml1,zmlsigma,mu1,alt01,zsg,nlvl &
                ,zsg_full,dzsg_full,zlvls,zfull,.FALSE.)
        END IF
        ...
        IF(.NOT.RAMSFLG)THEN
           ...
           alt01add(KZM+1)=alt01(KZM)
           alt02add(KZM+1)=alt02(KZM)
           ...
           DO I=KZM+2,NLVLADD
              ...
              TL(I)=TLPREV(I-1)
              DENS(I)=DENSPREV(I-1)
              alt01add(I)=alt01(I-1)
              alt02add(I)=alt02(I-1)
              UBAR(I)=UUPREV(I-1)
              VBAR(I)=VVPREV(I-1)
        ...
        if (vcoord .eq. 2) call compute_zsgfull(zsgadd,nlvladd,zsg_fulladd,dzsg_fulladd)
     ...
     ELSE
        ...
        IF(VEGHT.LE.9.0)THEN
           VEGHTZ=VEGHT*ZML2
           if (VEGHTZ.gt.ZMDL*0.9) then
              print*,'VEGHTZgtZMDL*0.9:Adjusting VEGHTZ=',VEGHT,' * ',ZML2,' to 0.9*ZMDL=',ZMDL !jmh 20150806 to prevent index error in ZADD below
              VEGHTZ=0.9*ZMDL
           endif
        ELSE
           VEGHTZ=VEGHT
        ...
        elseif (vcoord .eq. 2) then
           call agl2eta(veghtz,veghtsigma,mu2,alt02,zsg,nlvl &
                ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
           call agl2eta(zml2,zmlsigma,mu2,alt02,zsg,nlvl &
                ,zsg_full,dzsg_full,zlvls,zfull,.FALSE.)
        END IF
        ! JCL:(11/14/02)determine vertical index of mixed-layer ht
        ! CHG(09/22/03) don't do this for RAMS
     ...
     KZ=1
     DO WHILE(KZ.LE.NLVL .AND. ZSGADD(KZ+1).GT.ZPOS)
     ...
     IF(RAMSFLG)THEN
        KZ=1
        DO WHILE(KZ.LE.NLVL .AND. ZSGADD(KZ).GT.ZPOS)
           KZ=KZ+1
        END DO
        KT=KZ
     ...
     ! get Z profile (veght in meters above ground already computed above)
     if (vcoord .eq. 1) then
        ! zadd is the height at layer midpoint, zsg
        ZADD(1:NLVL)=(1.0-ZSGADD(1:NLVL))*(ZMDL-ZSFC)
     elseif (vcoord .eq. 2) then
        if (ktt .eq. 1) then
           call eta2agl(1.0,dummy,mu1,alt01add,zsgadd,nlvladd &
                ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.TRUE.)
        else
           call eta2agl(1.0,dummy,mu2,alt02add,zsgadd,nlvladd &
                ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.TRUE.)
        end if
        ! store layer top height in zadd (same as for RAMS)
        ! Note: only storing the first NLVL out of the NLVLADD=NLVL+1 heights
        !       ok since veghtz should never get close to model top
        zadd(1:nlvl)=zlvlsadd(1:nlvl)
     end if

     KV=1
     DENSINT=0.0
     ZADDOLD=0.0
     IF(RAMSFLG .or. vcoord .eq. 2)THEN
     ...
     DO WHILE(CUMTT.LT.ABS(DTm)/2.0)
        ...
        if (vcoord .eq. 2) &
             call eta2agl(zpos,zposz,mu2,alt02add,zsgadd,nlvladd &
             ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.FALSE.)
        ...
        IF(WPRIME.GT.0.0)THEN
           ...
           elseif (vcoord .eq. 2) then
              dist=abs(zposz-zlvlsadd(kt))/60.0
           end if
        END IF
        ...
        IF(WPRIME.LE.0.0)THEN
           ...
           ELSE
              ...
              elseif (vcoord .eq. 2) then
                 dist=abs(zposz-zlvlsadd(kb))/60.0
              end if
           END IF
        END IF
        ...
        IF((KUBLFLAG.EQ.1.AND.WPRIME.LT.0.0).OR.                       &
           ...
           IF(DIST.GE.ABS(DELT*WPRIME))THEN
              ...
              elseif (vcoord .eq. 2) then
                 zposz=zposz+wprime*delt*60.0
                 if (ktt .eq. 1) then
                    call agl2eta(zposz,zpos,mu1,alt01add,zsgadd,nlvladd &
                         ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.FALSE.)
                 else
                    call agl2eta(zposz,zpos,mu2,alt02add,zsgadd,nlvladd &
                         ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.FALSE.)
                 end if
              end if
  ...
  elseif (vcoord .eq. 2) then
     call eta2agl(zpos,zzold,mu2,alt02add,zsgadd,nlvladd &
          ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.FALSE.)
  end if
  ...
  IF (CONVDUR > 0 .AND. ZLOC > MAX1(ZML1,ZML2) .AND. ZLOC > ZZOLD) THEN
     ...
     elseif (vcoord .eq. 2) then
        call agl2eta(zz,zpos,mu2,alt02add,zsgadd,nlvladd &
             ,zsg_fulladd,dzsg_fulladd,zlvlsadd,zfulladd,.FALSE.)
     end if
  END IF

...
! WRF pdhs_sfc-pdhs_top
  if (vcoord .eq. 2) then
     CALL ADV2NT (mu(:,:,k1),XP,YP,var1,GLOBAL,NXP,NYP)
     CALL ADV2NT (mu(:,:,k2),XP,YP,var2,GLOBAL,NXP,NYP)
     METO%munext = (VAR2-VAR1)*TF+VAR1
  end if
...
! CHG(09/16/03) don't use this for RAMS
            IF (.NOT.RAMSFLG) THEN
               if (.not. awrfflg) then
                  if (fluxflg) then
                     ! time-averaged u,v: (NOTE: SUX,SUY,SVX,SVY set by metini according to grid)
                     IF (BACK) THEN
...
!dwen(20090826)                     METz%VVNEXT(KL) = VAR2
                  ...
                  if (vcoord .eq. 2) then
                     !Store the inverse density needed for the hydrostatic eqn
                     CALL ADV3NT (ALT0(:,:,:,k1),XP,YP,ZK,var1,GLOBAL,NXP,NYP)
                     CALL ADV3NT (ALT0(:,:,:,k2),XP,YP,ZK,var2,GLOBAL,NXP,NYP)
                     METz(kl)%ALT0next = (VAR2-VAR1)*TF+VAR1
                  end if
               end if !awrfflg
...
!dwen(20090826)           METz%DMASSNEXT(KL) = METz%DMASSNEXT(KL)*2.0/(DENS2+DENS1)     &
            if (vcoord .eq. 1) then
! NOTE: the factor multiplying dmassnext is in error (but is close to 1 kg/m3 near the ground)
!               METz(kl)%DMASSNEXT = METz(kl)%DMASSNEXT*2.0/(DENS2+DENS1)     &
!       the correct formula is (consistent with advpnt and prfcom):
!               METz(kl)%DMASSNEXT = METz(kl)%DMASSNEXT     &
               METz(kl)%DMASSNEXT = METz(kl)%DMASSNEXT     &
                    +((DENS2-DENS1)/((DENS2+DENS1)/2.0))/ABS(MTIME(2)-MTIME(1))
            elseif (vcoord .eq. 2) then
               if (kl .eq. 1) then
                  CALL ADV2NT(mu(:,:,K1),XP,YP,mu1,GLOBAL,NXP,NYP)
                  CALL ADV2NT(mu(:,:,K2),XP,YP,mu2,GLOBAL,NXP,NYP)
               end if
               METz(kl)%DMASSNEXT=METz(kl)%DMASSNEXT+                         &
                    ((mu2-mu1)/((mu2+mu1)/2.0))/ABS(MTIME(2)-MTIME(1))
            end if

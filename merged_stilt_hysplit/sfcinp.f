...
!    land use file
     ...
     IF(QFILE)THEN
        OPEN(255,FILE='LANDUSE.ASC',ACCESS='DIRECT',                       &
             RECL=LREC,FORM='UNFORMATTED',ACTION='READ')
        TFILE=.TRUE.
     ELSE
        INQUIRE(FILE=FDIR(1:KLEN)//'LANDUSE.ASC',EXIST=QFILE)
        IF(QFILE)THEN
           OPEN(255,FILE=FDIR(1:KLEN)//'LANDUSE.ASC',ACCESS='DIRECT',       &
                RECL=LREC,FORM='UNFORMATTED',ACTION='READ')
           TFILE=.TRUE.
        ELSE
...
!       load new data into buffers if record different from previous
        KREC=JREC
        print*,'Reading LANDUSE.ASC'
        IF(TFILE) READ(255,REC=JREC) TDAT  
!        print*,'WARNING, reading rec=35 from LANDUSE.ASC'
        print*,'End reading LANDUSE.ASC'
...
!    print*
!    print*,'WARNING: USING HARD-CODED VALUE OF 2 (AGRICULTURE) FOR LAND USE'
!    print*
!    LUSE=2

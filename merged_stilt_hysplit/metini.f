...
SUBROUTINE METINI(KDEF,KZMIX,TVMIX,KBLS,KBLT,NGRD,NTIM,OLAT,IBYR,    &
     IBMO,IBDA,IBHR,IBMN,BACK,KVEL,KFOR,vcoord,vcoords_iu)
  ...
  INTEGER,   INTENT(IN)    :: vcoord               ! vertical coordinate option
  INTEGER,   INTENT(IN)    :: vcoords_iu           ! unit number for vcoords_par input
  ...
  character (len=256) :: vcoords_fname
  integer, parameter :: vcoords_npar=8 !c1h, c2h, c3h (bh), c4h, c1f, c2f, c3f (bf), c4f
  integer :: vcoords_nlev, vcoords_mlev, ret_code, kret
  real, allocatable, save :: vcoords_pars(:,:)
  !*******************************************
  ...
  vcoords_fname = 'hybrid_wrfvcoords.txt'
  ! set all grid numbers to missing
  DO KG=1,MGRD
     DO KT=1,MTIM
        GRID(KG,KT)%NUMBER=-99
        GRID(KG,KT)%GBLDAT='no' !cg 20190821 need to initialize; before logical was initialized as FALSE
        GRID(KG,KT)%GLOBAL='no' !cg 20190821 need to initialize; before logical was initialized as FALSE
     END DO
  END DO
  ...
  DO KG=1,NGRD
     DO KT=1,NTIM
        ...
        WRITE (KF21,*) "NOTICE metini: For kg,kt= ",kg,kt," reading ",trim(FILE(KG,KT)%METEO)," in ",trim(FILE(KG,KT)%DIR)
        CALL METSET(KG,KT,KFOR)

        if (GRID(KG,KT)%model_id .eq. '9WRF' .or. GRID(KG,KT)%model_id .eq. 'NWRF') then
           if (vcoord .ne. 1) then
              write (*,*) 'ABORT: vcoord=',vcoord,' is not supported for WRF hybrid coordinates (9WRF/NWRF)'
              STOP 900
           endif
           ! arl file from WRF run with hybrid vertical coordinate, get needed vertical constants
           if (.not. allocated(vcoords_pars)) then
              vcoords_mlev=0
              write (*,*) 'First call to read_vcoords_pars to read parameters'
              call read_vcoords_pars(trim(file(kg,kt)%dir)//trim(vcoords_fname), &
                   & vcoords_iu,vcoords_pars,vcoords_nlev, drec(kg,kt)%wrfptop, &
                   & vcoords_mlev,vcoords_npar,ret_code)
              if (ret_code .eq. 0) then
                 write (*,*) 'ERROR metini: Unexpected successfull return from read_vcoords_par for vcoords_mlev=',vcoords_mlev
                 STOP 900
              elseif (ret_code .eq. -1) then
                 ! Retrieved no of levels, reallocate array and read them in:
                 vcoords_mlev=vcoords_nlev
                 deallocate(vcoords_pars,STAT=kret)
                 ALLOCATE (vcoords_pars(vcoords_mlev,vcoords_npar),STAT=kret)
                 IF(kret.ne.0)THEN      
                    WRITE(*,*)'ERROR metini: memory allocation - vcoords_pars',KRET
                    STOP 900
                 END IF
                 write (*,*) 'Second call to read_vcoords_pars to read parameters'
                 call read_vcoords_pars(trim(file(kg,kt)%dir)//trim(vcoords_fname), &
                      & vcoords_iu,vcoords_pars,vcoords_nlev, drec(kg,kt)%wrfptop, &
                      & vcoords_mlev,vcoords_npar,ret_code)
              else
                 ! Cannot read parameters, abort
                 write (*,*) 'ABORT: Initial call to read_vcoords_pars return code=',ret_code
                 STOP 900
              end if
           endif
           ! Store hybrid WRF vertical coordinate parameters in DREC for this grid:
           deallocate(drec(kg,kt)%wrfvcoords,STAT=kret)
           allocate(drec(kg,kt)%wrfvcoords(vcoords_mlev,vcoords_npar),stat=kret)
           IF(kret.ne.0)THEN      
              WRITE(*,*)'ERROR metini: memory allocation - drec(kg,kt)%wrfvcoords',KRET
              STOP 900
           END IF
           drec(kg,kt)%wrfvcoords(:,:) = vcoords_pars(:,:)
        endif
        ...
        IF(GRID(kg,kt)%model_id(2:4) .eq. 'WRF') THEN
           DREC(KG,KT)%WRF=1  
        ...
        IF(KVEL.EQ.6)THEN 
           IF(.NOT.(GRID(KG,KT)%MODEL_ID.EQ.' ETA'.OR.   &
              ...
              WRITE(*,*)'Input data file kg, kt, model_id: ',kg,kt,GRID(KG,KT)%MODEL_ID 
              STOP 900
           END IF
        END IF
        ...
        IF(.NOT.(DREC(KG,KT)%SHGT.OR.DREC(KG,KT)%PRSS))THEN
           WRITE(*,*)'*ERROR*: metini kg, kt, model_id:',kg,kt,GRID(KG,KT)%MODEL_ID,' - input meteorology requires either'
        ...
        WRITE (KF21,*) "MODEL_ID: ",GRID(KG,kt)%MODEL_ID
        WRITE (KF21,*) "TANG_LAT: ",GRID(KG,kt)%TANG_LAT
        WRITE (KF21,*) "REF_LAT:  ",GRID(KG,kt)%REF_LAT
        WRITE (KF21,*) "REF_LON:  ",GRID(KG,kt)%REF_LON
 ...
 !          GRID(KG,kt)%GLOBAL = .FALSE.
           ...
           write(KF21,*) "metini: Using WPS geolocation routines with proj_code: ",iproj
        ...
        ELSE
           ...
           IF(GRID(KG,KT)%MODEL_ID.EQ.'RAMS') THEN
              WRITE (KF21,*) "metini: Using RAMS grid mapping (SOBSTR)"
!             initialize grid conversion variable (into gbase)
           ...
           ELSE
              WRITE (KF21,'(a,l10)') "not RAMS, latlon flag=",GRID(KG,KT)%LATLON, &
                   & ' gbldat=',GRID(KG,KT)%LATLON,' prime=',GRID(KG,KT)%PRIME
...
!             initialize grid conversion variable (into gbase)
           ...
           END IF
        ENDIF
        WRITE (KF21,*) "SYNC_XP,SYNC_YP:",GRID(KG,KT)%SYNC_XP,GRID(KG,KT)%SYNC_YP
        WRITE (KF21,*) "SYNC_LON,SYNC_LAT:",GRID(KG,KT)%SYNC_LON,GRID(KG,KT)%SYNC_LAT
        WRITE (KF21,*) "SIZE,NX,NY:",GRID(KG,KT)%SIZE,GRID(KG,KT)%NX,GRID(KG,KT)%NY
        !*********************************************
        !
        !-------------------------------------------------------------------------------
...
! K2 was being used without being defined, so restructured slightly to correct
! Also, array indices were incorrect: GRID(K2,KT) changed to GRID(KG,K2) 

!          IF(GRID(KG,KT)%NUMBER.GE.0.AND.GRID(K2,KT)%NUMBER.GE.0)THEN
           IF(GRID(KG,KT)%NUMBER.GE.0)THEN

              DO K2=(KT+1),NTIM
! new line added in here with reformulated K2 test:
                 IF(GRID(KG,K2)%NUMBER.GE.0)THEN
           ...
           WRITE (KF21,*) "metini: Reordered kg,kt= ",kg,kt, &
                " SIZE,NX,NY: ",GRID(KG,KT)%SIZE,GRID(KG,KT)%NX,GRID(KG,KT)%NY, &
                " file: ",trim(file(kg,kt)%meteo)
        END DO
     END DO
  ...
  contains
    ...
    subroutine read_vcoords_pars(fname,iu,pars,nlev,ptop, &
         & mlev,npar,ret_code)
      implicit none
      character (len=*), intent(in) :: fname
      integer, intent(in) :: iu
      real, intent(out) :: pars(:,:), ptop
      integer, intent(in) :: mlev, npar
      integer, intent(out) :: nlev, ret_code

      ! automatic arrays
      real :: oneline(npar)
      ! Other local variables
      INTEGER            :: ierr, iline, j, k, nparin
      ...
      !    the number of levels, ptop
      ret_code=3
      iline = 1
      write(reason,'(a,i4,2a)') 'Error reading line',iline,' from file: ',trim(fname)
      read(iu,*,iostat=ierr) nlev, nparin, ptop
      if (ierr .ne. 0) goto 900
      if (nlev .gt. mlev) then
         write(reason,'(a,i4,3a,i4)') 'Specified number of levels:',nlev, &
              & ' in file: ',trim(fname), &
              & ' exceeds max allowable mlev=',mlev
         ret_code=-1
         goto 900
      end if
      if (nparin .ne. npar) then
         write(reason,'(a,i4,3a,i4)') 'Specified number of parameters:',nparin, &
              & ' in file: ',trim(fname), &
              & ' does not match parameter set in metini=',npar
         ret_code=-1
         goto 900
      end if
      ...
      ! Read one more line (comma or space delimited names of parameters)
      iline=iline+1
      write(reason,'(a,i4,2a)') 'Error reading header: line',iline,' from file: ',trim(fname)
      read(iu,'(a)',iostat=ierr) line
      if (ierr .ne. 0) goto 900

      ! Read in data (npar values per line, list-directed I/O)
      ret_code=5
      do k=1,nlev
         iline=iline+1
         write(reason,'(a,i4,2a)') 'Error reading data: line',iline,' from file: ',trim(fname)
         read(iu,*,iostat=ierr) (pars(k,j),j=1,npar)
         if (ierr .ne. 0) goto 900
      end do
      ...
      ! Print out results:
      write (*,'(2a)') 'Read_vcoords_par: hybrid wrf vertical coords parameters as determined from file :',&
           & trim(fname)
      goto 950

      ! error exit:
900   continue
      write (*,'(a/1x,a)') 'read_vcoords_pars: reading from file failed:',&
           & trim(reason)
...
950   continue
      ...
      if (ierr .ne. 0) then
         write (*,*) 'read_vcoords_pars warning: error closing file: '//trim(fname)
      end if

      if (ret_code .eq. 0 .or. ret_code .gt. 3) then
         write (*,'(1x,a,i4)')    'Number of levels: ',nlev
         write (*,'(1x,a,g15.6)') 'ptop:             ',ptop
         if (ret_code .eq. 0) then
            write (*,'(1x,a)') line
            write (line,'(a,i1,a)') '(1x,',npar,'g15.6,)'
            write (*,line) ((vcoords_pars(k,j),j=1,npar),k=1,nlev)
         end if
      end if

      return
    end subroutine read_vcoords_pars

...
!dwen(20090822): add conc in argument list.
 ...
 SUBROUTINE ADVPNT(METZ,METO,BACK,VMIX,CDEP,RDEP,TRAJ,KSFC,TKERD,TKERN,HDWP,   &
                   XP,YP,ZP,JET,DT,TRATIO,KG,KT,NGRD,NTIM,ZSG,NLVL,zsg_full,dzsg_full,ZMDL,KVEL,  &
...
! Note: VMIX=.TRUE. and TRAJ=.FALSE. hardwired in all advpnt calls in hymodelc.f90
!       CDEP and RDEP initialized in DEPSET (default: .FALSE.)
...
!dwen(20090813) *********************
  real                     :: dens1,dens2,mu2,mu1,tmp_mu
  logical                  :: awrfflg,fluxflg,deepflg,shallflg,geosflg
  real,        allocatable :: tlk1(:),tlk2(:),sigwk1(:),sigwk2(:),zlvls(:),zfull(:),tmp_alt0(:)
...
!-------------------------------------------------------------------------------
  ...
  SAVE LX1,LY1,NXS,NYS,MTIME,FTIME,FHOUR,fluxflg,deepflg,shallflg
...
!dwen(20090820) ********************
  IF(.NOT.ALLOCATED(tlk1))THEN
     ...
     ALLOCATE(tlk1(nlvl),tlk2(nlvl),sigwk1(nlvl),sigwk2(nlvl), &
          zlvls(nlvl),zfull(nlvl),tmp_alt0(nlvl),STAT=kret)
     IF(kret.NE.0)GOTO 9000
  end if
! **********************************
...
!                                                (this array used to store alt0 for AWRF)
...
!dwen        X(:,:,:,K1,KG))
     ...
     CALL PRFCOM(TKERD,TKERN,KG,KT1,KSFC,GX(:,:,KG),GY(:,:,KG),Z0(:,:,KG),     &
        ZT(:,:,KG),NXS(KG),NYS(KG),NZS,ZMDL,ZSG,NLVL,zsg_full,dzsg_full,VMIX,KMIXD,KMIX0,         &
        ZI(:,:,K1,KG),                                                         &
...
!dwen(20090826)               METz%VVPREV(KL)=VAR2
            ...
            if (fluxflg) then
               ...
               CALL ADV3NT(V(:,:,:,K2,kg),XX,YY,ZK,                  &
                    var2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)
               METz(kl)%VVPREV=(VAR2-VAR1)*TF+VAR1
            end if !fluxflg-else-endif
         endif !awrf-else-endif
...
!        also need the density change term (kg/m3/min) as part of mass budget!
     ...
     &           ((DENS2-DENS1)/((DENS2+DENS1)/2.0))/DM
         elseif (vcoord .eq. 2) then
            if (kl .eq. 1) then
               CALL ADV2NT(mu(:,:,K1,kg),XX,YY,                  &
                    mu1,GRID(kg,kt1)%GLOBAL,GRID(kg,kt1)%NX,GRID(kg,kt1)%NY)
               CALL ADV2NT(mu(:,:,K2,kg),XX,YY,                  &
                    mu2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)
               METO%muprev = (mu2-mu1)*TF+mu1
               ! For eta2agl for zmix computation below, use nearest neighbor interpolation for mu:
               CALL ADV2NT(mu(:,:,K1,kg),anint(XX),anint(YY),                  &
                    var1,GRID(kg,kt1)%GLOBAL,GRID(kg,kt1)%NX,GRID(kg,kt1)%NY)
               CALL ADV2NT(mu(:,:,K2,kg),anint(XX),anint(YY),                  &
                    var2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)
               tmp_mu = (var2-var1)*TF+var1
            end if
            METz(kl)%DMASSPREV=METz(kl)%DMASSPREV+                         &
     &           ((mu2-mu1)/((mu2+mu1)/2.0))/DM
            CALL ADV3NT(tlrams(:,:,:,K1,kg),XX,YY,ZK,                  &
                 var1,GRID(kg,kt1)%GLOBAL,GRID(kg,kt1)%NX,GRID(kg,kt1)%NY)
            CALL ADV3NT(tlrams(:,:,:,K2,kg),XX,YY,ZK,                  &
                 var2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)
            metz(kl)%alt0prev=(VAR2-VAR1)*TF+VAR1
            ! For eta2agl for zmix computation below, use nearest neighbor interpolation for
            ! WRF inverse density before advection
            CALL ADV3NT(tlrams(:,:,:,K1,kg),anint(XX),anint(YY),ZK,                  &
                 var1,GRID(kg,kt1)%GLOBAL,GRID(kg,kt1)%NX,GRID(kg,kt1)%NY)
            CALL ADV3NT(tlrams(:,:,:,K2,kg),anint(XX),anint(YY),ZK,                  &
                 var2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)
            tmp_alt0(kl)=(VAR2-VAR1)*TF+VAR1
         end if
...
! NOTE: for consistency with zml computation, use nearest-neighbor, not horizontal interpolation,
!       for terrain height (ZGRD)
      ...
      elseif (vcoord .eq. 2) then
         call eta2agl(zsg(1),zagl,tmp_mu,tmp_alt0,zsg,nlvl &
...
! dwen(20090810)   DVDZ and DUDZ are useless in ADVIEC, removed them
     if (vcoord .eq. 1) then
        zagl=(1.0-ZZ)*(ZMDL-meto%ZTER) 
     elseif (vcoord .eq. 2) then
        ! Recompute AGL heights for use within adviec (using bilinearly interpolated
        ! mu and alt0)
        call eta2agl(zz,zagl,meto%muprev,metz(:)%alt0prev,zsg,nlvl &
             ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
     end if
     ...
     CALL ADVIEC(U(:,:,:,:,KG),V(:,:,:,:,KG),W(:,:,:,:,KG),d(:,:,:,:,kg),      &
          ...
          zagl,zlvls(1),wwout,awrfflg,fluxflg,zsg,ramsflg,dead,vcoord)
     kgold = 0
     if (dead) then
...
!    lagrangian isobaric sampling
     CALL ADVISO(U(:,:,:,:,KG),V(:,:,:,:,KG),P(:,:,:,:,KG),ZSG,                &
          K1,K2,NLVL,MTIME,JET,ZMDL,XX,YY,ZZ,METO%ZNDX,DT,TRATIO,BACK,         &
          GRID(KG,KT1)%GLOBAL,GRID(KG,KT1)%NX,GRID(KG,KT1)%NY,                 &
          METO%UVEL,METO%VVEL,RAMSFLG,vcoord)
...
!    NOTE: Need to add 1 to xxmax/yymax, because adv3nt (called below) checks
!          and uses int(xx+1)/int(yy+1) (only seems to be a problem with geos?)
!          Message printed to stdout with "adviec" string needed for error-checking script
     xx_min = min(min(xx,xx+sux),xx+svx)
     xx_max = max(max(xx,xx+sux),xx+svx)
     if (geosflg) xx_max = xx_max+1
     yy_min = min(min(yy,yy+suy),yy+svy)
     yy_max = max(max(yy,yy+suy),yy+svy)
     if (geosflg) yy_max = yy_max+1
     IF(xx_min .LT. 1.0 .OR. yy_min .LT. 1.0 .OR. xx_max .GT. FLOAT(NXS(KG)) .OR.                     &
        ...
        WRITE(KF21,*)' NOTICE advpnt: particle with xx_min, xx_max, yy_min, yy_max ', &
             ...
             & kg,nxs(kg),nys(kg)
        WRITE(*,*)' NOTICE advpnt: after adviec, terminated particle with xx_min, xx_max, yy_min, yy_max ', &
             & xx_min, xx_max, yy_min, yy_max,' off subgrid for kg,nxs,nys ', &
             & kg,nxs(kg),nys(kg)
        KG=0
        KRET=1
...
!               hf: sensible heat flux ;   sf: stability function
  ...
  CALL ADVMET(METZ,METO,BACK,VMIX,CDEP,RDEP,TRAJ,XX,YY,JTIME,MTIME,            &
              ...
              sm(:,:,:,kg),ramsflg,ecmflg,awrfflg,fluxflg,tlrams(:,:,:,:,kg),  &
              mu(:,:,:,kg),vcoord)
!
...
! calculate true agl heights [m] of model levels:
      ...
      elseif (vcoord .eq. 2) then
!     compute agl heights after advection, using nearest neighbor mu and alt0:
         CALL ADV2NT(mu(:,:,K1,kg),anint(XX),anint(YY),                  &
              var1,GRID(kg,kt1)%GLOBAL,GRID(kg,kt1)%NX,GRID(kg,kt1)%NY)
         CALL ADV2NT(mu(:,:,K2,kg),anint(XX),anint(YY),                  &
              var2,GRID(kg,kt2)%GLOBAL,GRID(kg,kt2)%NX,GRID(kg,kt2)%NY)

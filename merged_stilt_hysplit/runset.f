...
!
! USAGE:  CALL RUNSET(SPOT,NLOC,NHRS,maxpage,NGRD,NTIM,ZMDL,ZDATA,BACK,IUNIT,KVEL)
!
...
SUBROUTINE RUNSET(SPOT,NLOC,NHRS,maxpage,NGRD,NTIM,ZMDL,ZDATA,BACK,IUNIT,KVEL)
...
!-------------------------------------------------------------------------------
  ...
  INTEGER,   INTENT(OUT)   :: maxpage   ! maximum age (abs value) for particles
...
!     READ(IUNIT,*)SPOT(N)%OLAT,SPOT(N)%OLON,SPOT(N)%OLVL
      ...
      CALL DECODR(IUNIT,SPOT(N)%OLAT,SPOT(N)%OLON,SPOT(N)%OLVL,     &
                        SPOT(N)%QTRM,SPOT(N)%AREA,rptyp,SPOT(N)%dxyp,SPOT(N)%dzp)
      SPOT(N)%ptyp=nint(rptyp)
      IF(IUNIT.EQ.5)WRITE(KF22,'(3F10.3)')SPOT(N)%OLAT,SPOT(N)%OLON,SPOT(N)%OLVL
!!$      WRITE(KF21,'(a,5F10.3,i3)') 'debug runset: spot= ',SPOT(N)%OLAT,SPOT(N)%OLON,SPOT(N)%OLVL,&
!!$           SPOT(N)%qtrm,SPOT(N)%area,SPOT(N)%ptyp
...
!-------------------------------------------------------------------------------
  ...
  rnhrs=nhrs
  rmaxpage=0
  call decodr(iunit,rnhrs,rmaxpage)
  nhrs=nint(rnhrs)
  maxpage=nint(abs(rmaxpage))
  if (maxpage < 1) maxpage=60*abs(nhrs)

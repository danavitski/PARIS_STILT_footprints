...
SUBROUTINE CONSUM(conc,numgrd,plat,plon,dt,jet,zmdl,zsfc,kmsl,                &
                  cgsize,mass,dept,zpos,sigh,sigw,hdwp,ptyp,csum,vcoord)
...
!-------------------------------------------------------------------------------
    ...
    if (vcoord .ne. 1 .and. (HDWP .EQ. 1 .OR. HDWP .EQ. 2 .or. &
         HDWP .EQ. 0 .OR. HDWP .EQ. 3 .OR. HDWP .EQ. 4) .and. nwarn .eq. 0) then
       !TBD_sigmap: add support here
       nwarn = nwarn + 1
       write (*,*) 'WARNING consum: vcoord,hdwp=',vcoord,hdwp, &
            ', vertical extent computations suspect'
    end if

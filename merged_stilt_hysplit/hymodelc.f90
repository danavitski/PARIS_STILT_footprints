...
PROGRAM HYMODELC
  ...
  use module_ext_zsg, ONLY: read_zsg, ind_zsg
  use module_wrfeta, ONLY: eta2agl, agl2eta, compute_zsgfull
  use module_medzpos, ONLY: compute_pctzpos
  ...
  ! Choice of vertical coordinate: 1-sigma-z; 2-sigma-p (WRF);
  INTEGER :: vcoord=1
  ...
  INTEGER, allocatable              :: countnparout(:)
  ...
  integer                           :: hroutmin
  real                              :: mindveg !min value of veght for dynamic veght estimation
  real                              :: dyn_fac !turn off dyn_veg once median particle veght exceeds dyn_fac*veght
  integer                           :: pctveght ! percentile to use for dynamic veght estimation
  ...
  REAL, ALLOCATABLE :: zsg  (:), zsg_full(:), dzsg_full(:)         ! model sigma levels
  REAL, ALLOCATABLE :: zlvls  (:), zfull (:)         ! AGL  at sigma layers/levels
  logical :: compute_zlvls
  ...
  !MR 20140115 added lon lat boundaries
  REAL     :: minplon=-1000., minplat=-1000., maxplon=1000., maxplat=1000.
  logical  :: in_latlon_box
  ...
  INTEGER  :: iyr,imn,kp,kpt,kp_loop,i,j,k,k1,k2,iunit,ibda,ibhr,ibmn,nstr,ichem,cmass 
  INTEGER  :: kdef,mgmin,nloc,ibyr,numspl,ktyp,kmsl,klen,initd,khmax,cpack 
  INTEGER  :: n,jet,kg,numtyp,nzp,kpm,nyp,kgrid,numgrd,nxp,nhrs,maxpage,ngrd,ntim
  ...
  integer :: clock_cycle1, clock_cycle2, clock_rate, clock_max, clock_start
  real :: elapsed, elapsed_min, printed_elapsed, maxprint_elapsed=100, factor_elapsed=10
  real :: veghtz, mindvegz, sveghtz, medzposz
  integer :: jtyp
  logical, allocatable :: dyn_veg(:), pdyn_veg(:)
  real, allocatable :: pveght(:), pveghtsum(:), pzmlsum(:), medzpos(:)
  integer, allocatable :: npveght(:)
  ...
  NAMELIST/SETUP/INITD,KHMAX,NUMPAR,MAXPAR,MAXDIM,QCYCLE,P10F,KMIXD,KMIX0,     &
       ...
       iconvect,winderrtf,ivmax,umaxi,varsiwant,vcoord, &
       minplon,minplat,maxplon,maxplat,mindveg,dyn_fac,pctveght,hroutmin
  ...
  OUTDT=0.0
  hroutmin=0 ! by default, no special output frequency for beginning of back trajectory
  ! JCL:ht [fraction of PBL ht or m] below which a particle's time spent is tallied;
  !        If <=9.0, then specifies fraction of PBL ht
  VEGHT=0.5
  mindveg=0
  dyn_fac=1.5 !turn off dyn_veg once median particle veght exceeds dyn_fac*veght
  pctveght=50 !use median of particle heights for dynamic veght estimation
  ...
  WRITE(*,'(a,i10)') ' NOTICE   main: NLOC= ',NLOC
  ...
  CALL RUNSET(SPOT,NLOC,NHRS,maxpage,NGRD,NTIM,ZMDL,ZDATA,BACK,IUNIT,KVEL)
  WRITE(*,*) 'LON/LAT Boundaries (minlon, minlat, maxlon, maxlat)'
  WRITE(*,*) minplon,minplat,maxplon,maxplat
  if (vcoord .ne. 1 .and. kvel .gt. 0) then
     write (*,*) 'ERROR hymodelc: kvel=',kvel,' not supported for vcoord=',vcoord
     write (*,*) ' need to change either (1) vcoord=1 or (2) kvel=0'
     stop 900
  end if
  ...
  CALL METINI(KDEF,KZMIX,TVMIX,KBLS,KBLT,NGRD,NTIM,SPOT(1)%OLAT,SPOT(1)%IBYR, &
       SPOT(1)%IBMO,SPOT(1)%IBDA,SPOT(1)%IBHR,SPOT(1)%IBMN,BACK,KVEL,KFOR,vcoord,KFJCLMSG)
  ...
  if (vcoord .eq. 2) then
     if (awrfflg) then
        NLVL=GRID(KG,kt)%NZ-1
        deallocate(zsg)
        ALLOCATE (zsg(nlvl),zlvls(nlvl),zfull(nlvl),STAT=kret)
        IF(kret.ne.0)THEN      
           ECODE='Sigma profile - vcoord=2 awrf branch'
           WRITE(*,*)'ERROR hymodelc: memory allocation - ',ECODE,KRET
           STOP 900
        end IF
        ! Set ZSG to eta half-level values (DREC%HEIGHT(1:NLVL) contains eta full level values)
        ZSG(1:NLVL)=0.5*(DREC(KG,kt)%HEIGHT(1:NLVL)+DREC(KG,kt)%HEIGHT(2:(NLVL+1)))
        !debug:
        print *,'NLVL, HEIGHT:',NLVL,DREC(KG,kt)%HEIGHT(1:NLVL)
        KSFC=1
        SFCL=DREC(KG,kt)%HEIGHT(KSFC+1)
     else
        WRITE(*,*)'WARNING hymodelc: invalid vcoord=2 for awrfflg=FALSE, reset to 1'
        vcoord = 1
     endif
  endif
  ...
  if (.not. RAMSFLG .and. vcoord .eq. 1) then
     ...
     elseif (ret_code .eq. -1) then
        ...
        write (*,*) 'Initial call return code=',ret_code,', cannot read in zsg levels'
     end if
     !         call read_zsg ('ZSG_LEVS.IN',45,zmdl,zsg,nzm,nlvl,ksfc,sfcl,aa,bb,cc)
  endif
  allocate(zsg_full(nlvl),dzsg_full(nlvl))
  call compute_zsgfull(zsg,nlvl,zsg_full,dzsg_full)
  ...
  ! NOTE: already checked in runset against zdata; redo here only for vcoord .eq. 1:
  if (vcoord .eq. 1) then
...
!!$          WRITE(*,*)'   Model domain - ',INT(DREC(KG)%HEIGHT(NLVL+1))
           ...
           STOP 'ERROR hymodelc: Start height above mdl domain'
        END IF
     END DO
  end if
...
!           IF(GRID(KG,KT)%GBLDAT)THEN
  ...
  do k=1,nloc
     WRITE(KFPARDAT,*)SPOT(k)%OLAT,SPOT(k)%OLON,SPOT(k)%OLVL
  end do
  ...
  ! add ptyp to output if numtyp>1:
  if (numtyp .gt. 1) then
     k2=0
     do k1=1,ivmax
        if (trim(varsiwant(k1)) .eq. 'ptyp' .or. &
            trim(varsiwant(k1)) .eq. 'PTYP') k2=k1
     end do
     if (k2 .eq. 0) then
        WRITE(*,*)' NOTICE   main: numtyp= ',numtyp,' > 1, adding ptyp to varsiwant'
        ivmax=ivmax+1
        IF(IVMAX.GT.100) STOP 'Too many output vars (IVMAX) in SETUP.CFG!'
        varsiwant(ivmax)='PTYP'
     else
        WRITE(*,*)' NOTICE   main: numtyp= ',numtyp,' > 1, ptyp already in varsiwant'
     end if
  end if

  ! allocate and initialize arrays for dynamic veght estimation
  allocate(dyn_veg(numtyp),pveght(numtyp),pveghtsum(numtyp),pzmlsum(numtyp),&
       medzpos(numtyp),npveght(numtyp),pdyn_veg(numtyp))
  dyn_veg(:)=mindveg .gt. 0
  pdyn_veg(:)=dyn_veg(:)
  ...
  ALLOCATE (dirt(numpol),countnparout(numpol),STAT=kret)  
  IF(kret.ne.0)THEN      
  ...
  CALL CONSET(CONC,ZMDL,NUMGRD,IUNIT,CGSIZE,CPACK,SPOT(1)%OLAT,SPOT(1)%OLON,   &
       SPOT(1)%IBYR,SPOT(1)%IBMO,SPOT(1)%IBDA,SPOT(1)%IBHR,BACK,vcoord)
  ...
  IF(NDUMP.GT.0)THEN
     ...
     IF(FTEST1)THEN
        ...
        CALL PARINP(JET,KG,KT,KPM,MASS,TLAT,TLON,XPOS,YPOS,ZPOS,SIGH,SIGU,   &
             SIGV,SIGW,HDWP,PAGE,PTYP,PGRD,NSORT,ZMDL,MAXPAR,NINIT,vcoord)
        ...
        DO J=1,KPM
           ...
           IF(ICHEM.EQ.4)THEN
              ...
              CALL METSUM(CONC,NUMGRD,XPOS(J),YPOS(J),METO%GDISX,METO%GDISX,      &
                   ...
                   SIGH(J),SIGW(J),HDWPX,PTYP(J),CSUM,vcoord)
           ELSE
              IF(CMASS.EQ.0)THEN
              ...
              ELSE
                 ...
                 CALL MASSUM(CONC,NUMGRD,TLAT(J),TLON(J),DT,JET,ZMDL,ZSFC,KMSL,      &
                      CGSIZE,MASS(:,J),ZPOS(J),PTYP(J),CSUM,vcoord)
              END IF
           END IF
        END DO
  ...
  DT=1.0 !use a single one-minute timestep for dummy advpnt calls (stay close to starting location)
  ...
  nloop : DO N=1,NLOC
     ...
     YTMP =SPOT(N)%YP
     ZTMP =SPOT(N)%ZP
     if (vcoord .eq. 2) ztmp=zsg(1) !use lowest model layer for dummy advpnt call
     KGRID=SPOT(N)%KG
     KTIME=KT
     ...
     CALL ADVPNT(METZ,METO,BACK,.TRUE.,CDEP,RDEP,.FALSE.,KSFC,TKERD,TKERN,     &
          ...
          ZSG,NLVL,zsg_full,dzsg_full,ZMDL,KVEL,KMIXD,KMIX0,UBAR,IFHR,KRET,                &
          conc,wwout,zicontroltf,nhrszi,zipresc,convdur(1),iconvect,    &
          ramsflg,ecmflg,xposprev,yposprev,vcoord)
     ...
     elseif (vcoord .eq. 2) then
        !    convert AGL to eta, using interpolated mu and alt0
        call agl2eta(spot(n)%olvl,spot(n)%zp,meto%muprev,metz(:)%alt0prev,zsg,nlvl &
             ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
     end if

     IF(KMSL.EQ.1 .or. vcoord .eq. 2 .or. METO%ZTER .ne. 0.0) THEN
        !       another dummy call to get correct initial pressure
        !       needed because initial ztmp was off for KMSL=1, zter /=0, or vcoord=2
        XTMP=SPOT(N)%XP
        YTMP=SPOT(N)%YP
        ZTMP=SPOT(N)%ZP
        ...
        ! The results from this call are not used at all: call not needed
        ! (otherwise, would need to repeat the ZP computation from above?)
  ...
  if (vcoord .eq. 2) then
     do n=1,nloc
        if (spot(n)%zp .lt. zsg(nlvl)) then
           WRITE(*,*)'ERROR hymodelc: Start height above mdl domain (sigma-p)'
           WRITE(*,*)'   Source - ',N,'    Height - ',SPOT(N)%OLVL,' Sigma - ',SPOT(N)%zp
           if (spot(n)%zp .le. -900) write (*,*) 'Dummy advpnt calls returned non-zero kret'
           WRITE(*,*)'   Model domain top sigma - ',zsg(nlvl)
           STOP 'ERROR hymodelc: Start height above mdl domain (sigma-p)'
        END IF
     end do
  end if
...
!           IF(.NOT.GRID(KG,KT)%GBLDAT)THEN
        ...
        IF(KRET.NE.0) then
           WRITE(KF21,*)'WARNING main: lagrangian release point n=',n, &
                ' has nonzero advpnt return code'
           if (vcoord .eq. 1) then
              WRITE(KF21,*)' proceeding with zero terrain for this point'
              METO%ZTER=0.0 
           elseif (vcoord .eq. 2) then
              WRITE(KF21,*)' disabling this point'
              LAGS(N)%RACM=0  ! turn off release for this location
              cycle
           end if
        end IF
        ...
        elseif (vcoord .eq. 2) then
           !    convert AGL to eta, using interpolated mu and alt0
           call agl2eta(lags(n)%rlvl,lags(n)%zp,meto%muprev,metz(:)%alt0prev,zsg,nlvl &
                ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
        end if
  ...
  DO KH=1,NHRS
     ...
     if (kh .eq. 1) then
        ...
        if (int(dt) .ne. dt_saved) then
           WRITE(KF21,*)' NOTICE   main: at kh, jet=',kh,jet,', New time step (min) ',INT(DT), &
                ' maxdt,TRATIO,CGSIZE,UMAX',maxdt,TRATIO,CGSIZE,UMAX
           dt_saved=int(dt)
        end if
     end if
     ...
     DO KS=1,NSTEP
           ...
           IF(KPM.GT.KPT)THEN
              ...
              DO KP_loop=KPT+1,KPM
                 ...
                 IF(KRET.EQ.0)THEN 
                    ...
                    END IF
                    if (vcoord .eq. 1) then
                       height=min(height,ZMDL-METO%ZTER) !keep below model top
                       ZPOS(KP)=1.0-height/(ZMDL-METO%ZTER)
                    elseif (vcoord .eq. 2) then
                       !    convert AGL to eta, using interpolated mu and alt0
                       call agl2eta(height,zpos(kp),meto%muprev,metz(:)%alt0prev,zsg,nlvl &
                            ,zsg_full,dzsg_full,zlvls,zfull,.TRUE.)
                       zpos(kp)=max(zsg(nlvl),zpos(kp))  !keep below model top
                    end if
                 ELSE
                    WRITE(KF21,*)'WARNING main: added particle kp_loop=',kp_loop, &
                         ' has nonzero advpnt return code'
                    if (vcoord .eq. 1) then
                       WRITE(KF21,*)' initial zpos was: ',zpos(kp), &
                            ' proceeding with returned ztmp=',ztmp
                       ZPOS(KP)=ZTMP 
                    else
                       WRITE(KF21,*)' deleting this particle'
                       kp=kp-1
                       cycle
                    end if
                 END IF !kret.eq.0
              end DO !kp_loop=kp+1,kpm
        ...
        ELSE !neither QFILE nor QTEMP
           ...
           CALL EMSPNT(SPOT,DIRT,NLOC,NUMTYP,KPM,INITD,DT,JET,NSORT,           &
                ...
                PGRD,QCYCLE,NUMPAR,MAXPAR,job_id,num_job,ichem,KEMIT,rseed,zsg,zsg_full,nlvl)
           if (kpm .ne. kpt) write (KF21,*) &
                'Notice main: after emspnt, kh,kt,dt,kpt,kpm,kret=',kh,kt,dt,kpt,kpm,KEMIT
        ...
        ! For positive hroutmin, output every timestep for first hroutmin minutes:
        writeout=writeout .or. ((kh-1)*60+ks*abs(int(dt)) .le. HROUTMIN)
        ...
        END IF !IF(KH.EQ.1.AND.KS.EQ.1.AND.(WINDERRTF.EQ.1.OR.WINDERRTF.EQ.3))
        !-----------------------------------------------------------
        ...
        IF(KH.EQ.1.AND.KS.EQ.1.AND.(WINDERRTF.EQ.2.OR.WINDERRTF.EQ.3))THEN
           ...
           END DO
           !IF(KH.EQ.1.AND.KS.EQ.1.AND.(WINDERRTF.EQ.2.OR.WINDERRTF.EQ.3))THEN
        END IF !IF(KH.EQ.1.AND.KS.EQ.1.AND.(WINDERRTF.EQ.2.OR.WINDERRTF.EQ.3))
        !
        !****************************************************************
        ...
        COUNTNPAROUT(:)=0
        ! dynamic veght estimation
        if (any(dyn_veg)) then
           if(kh .gt. 1 .or. ks .gt. 1) then
              ! compute mean veght from previous time step
              ! (either as height or fraction, according to veght): 
              pveght(:)=pveghtsum(:)/ &
                   merge(tsource=(/(1,i=1,numtyp)/),fsource=npveght(:),&
                    mask=npveght(:) .lt. 1)
              if (veght .lt. 9) then
                 pzmlsum(:)=pzmlsum(:)/&
                   merge(tsource=(/(1,i=1,numtyp)/),fsource=npveght(:),&
                    mask=npveght(:) .lt. 1)
                 pveght(:)=pveght(:)/&
                      merge(tsource=(/(1.0,i=1,numtyp)/),fsource=pzmlsum(:),&
                      mask=pzmlsum(:) .lt. 1.0)
              end if
              ! check for ending dynamic veght computations
              do jtyp=1,numtyp
                 dyn_veg(jtyp) = dyn_veg(jtyp) .and. &
                      pveght(jtyp) .lt. dyn_fac*veght
              end do
           endif
        end if
        !diagnostic printout:
        if (any(pdyn_veg .and. .not. dyn_veg)) then
           write (*,'(2(a,i5),a,g15.6)') 'Deactivating dyn_veg at kh=',kh,' ks=',ks,' t_minutes=',(ks-1.)*abs(dt)
           do jtyp=1,numtyp
              if (pdyn_veg(jtyp) .and. .not. dyn_veg(jtyp)) then
                 if (veght .lt. 9) then
                    write (*,'(a,i5,2(a,g15.6))') 'jtyp=',jtyp,' pveght=',pveght(jtyp),' pzmlsum=',pzmlsum(jtyp)
                 else
                    write (*,'(a,i5,2(a,g15.6))') 'jtyp=',jtyp,' pveght=',pveght(jtyp)
                 endif
              end if
           end do
        end if
!!$        !debugging printout:
!!$        write (*,*) 'kh=',kh,' ks=',ks,' t_minutes=',(ks-1.)*abs(dt)
!!$        write (*,*) ' dyn_veg=',dyn_veg(:)
!!$        write (*,*) ' pdyn_veg=',pdyn_veg(:)
!!$        if (kh .gt. 1 .or. ks .gt. 1) then
!!$           write (*,*) ' pveght=',pveght(:)
!!$           if (veght .lt. 9) write (*,*) ' pzmlsum=',pzmlsum(:)
!!$        end if
        pdyn_veg(:)=dyn_veg(:)
        if (any(dyn_veg)) then
           ! compute median particle height (sigma)
           ! (only model sigma values are available here)
           call compute_pctzpos(zpos, dyn_veg, pgrd, ptyp, medzpos, kpm, numtyp, pctveght)
!!$        !debugging printout:
!!$           write (*,*) ' medzpos=',medzpos(:)
           ! initialize arrays for mean veght computation
           pveghtsum(:) = 0.
           pzmlsum(:) = 0.
           npveght(:) = 0
        end if
        ploop : DO KPT=1,KPM
           ...
           compute_zlvls=.TRUE.  !initialize flag for vcoord=2 eta2agl conversions

           !       current computation grid index number
           KP=NSORT(KPT)

           !       skip terminated particles, but keep increasing their age:
           IF(PGRD(KP).EQ.0) then
              countnparout(ptyp(kp)) = countnparout(ptyp(kp)) + 1
              PAGE(KP)=SIGN( (ABS(PAGE(KP))+NINT(ABS(DT))), PAGE(KP) )
              CYCLE ploop
           endif
           ...
           call system_clock(clock_cycle1,clock_rate,clock_max)
           CALL ADVPNT(METZ,METO,BACK,.TRUE.,CDEP,RDEP,.FALSE.,KSFC,TKERD,TKERN,  &
                HDWPX,XPOS(KP),YPOS(KP),ZPOS(KP),JET,DT,TRATIO,PGRD(KP),          &
                KTIME,NGRD,NTIM,ZSG,NLVL,zsg_full,dzsg_full,ZMDL,KVEL,KMIXD,KMIX0,UBAR,IFHR,KRET,    &
                conc,wwout,zicontroltf,nhrszi,zipresc,convdur(kp),iconvect,       &
                ramsflg,ecmflg,xposprev,yposprev,vcoord)

           !****************************************************************
           ! print out advpnt elapsed times, for time steps with large elapsed times only
           ! adjust criterion for "large" to limit amount of printed output
           call system_clock(clock_cycle2,clock_rate,clock_max)
           elapsed=real(clock_cycle2 - clock_cycle1)/real(clock_rate)
           if (elapsed .lt. 0) elapsed=elapsed+real(clock_max)/real(clock_rate)
           elapsed_min=max(1./real(clock_rate),min(elapsed_min,elapsed))
           if (elapsed .gt. factor_elapsed*elapsed_min) then
              WRITE(KF21,*)' NOTICE   main: advpnt elapsed: ', &
                   kh,ks,jet,kp,elapsed
              printed_elapsed=printed_elapsed+1
              if (printed_elapsed .gt. maxprint_elapsed) then
                 printed_elapsed=0
                 factor_elapsed=2.*elapsed/elapsed_min
                 WRITE(KF21,*)' NOTICE   main: resetting elapsed_min, factor_elapsed: ', &
                   elapsed_min,factor_elapsed,kh,ks,jet,kp
                 elapsed_min=elapsed
              end if
           end if
           ...
           IF ((RAMSFLG .OR. ECMFLG .or. awrfflg) .AND. NTURB .EQ. 0 .AND. ICONVECTRAMS .EQ. 1)THEN
              !convert to normalized height (this uses zero terrain for some reason)
              if (vcoord .eq. 1) then
                 Z1=(1.0-ZPOS(KP))*ZMDL
                 if (awrfflg) then
                 ...
                 ELSE IF (ECMFLG) THEN                         ! arithm. mean of mid-level z values
                    do kk=1,nlvl-1
                       zprofm(kk) = 0.5d0*zmdl*(2d0-zsg(kk)-zsg(kk+1))
                    end do
                    zprofm(nlvl) = zmdl
                 else
              ...
              elseif (vcoord .eq. 2) then
                 !    convert eta to AGL, using interpolated mu and alt0
                 call eta2agl(zpos(kp),z1,meto%munext,metz(:)%alt0next,zsg,nlvl &
                      ,zsg_full,dzsg_full,zlvls,zfull,compute_zlvls)
                 compute_zlvls=.FALSE.
                 zprofm(1:nlvl)=zfull(1:nlvl)
              end if
              ...
              IF(RAUP.GT.0.0)THEN
                 ...
                 if (vcoord .eq. 1) then
                    ...
                    ZPOS(KP)=1.0-Z2/ZMDL
                 elseif (vcoord .eq. 2) then
                    ! convert agl back to eta
                    call agl2eta(z2,zpos(kp),meto%munext,metz(:)%alt0next,zsg,nlvl &
                         ,zsg_full,dzsg_full,zlvls,zfull,compute_zlvls)
                    compute_zlvls=.FALSE.
                 end if
           ...
           IF(WINDERRTF.EQ.1.OR.WINDERRTF.EQ.3)THEN
              ...
              DO KZZ=2,NLVL
                 ...
                 elseif (vcoord .eq. 2) then
                    if (compute_zlvls) then
                       call eta2agl(zsg(1),deltaz,meto%munext,metz(:)%alt0next,zsg,nlvl &
                            ,zsg_full,dzsg_full,zlvls,zfull,compute_zlvls)
                       compute_zlvls=.FALSE.
                    end if
                    deltaz=zlvls(kzz)-zlvls(kzz-1)
                 end if
                 RAUTO=EXP(-1.0*DELTAZ/ZCORLEN)
           ...
           IF (NTURB == 0) THEN
              ...
              if (dyn_veg(jtyp)) then
                 ! dynamic veght estimation:
                 ! convert medzpos to height agl
                 if (vcoord .eq. 1) then
                    MEDZPOSZ=(1.0-MEDZPOS(jtyp))*(ZMDL-ZSFC)
                 elseif (vcoord .eq. 2) then
                    call eta2agl(medzpos(jtyp),medzposz,meto%munext,metz(:)%alt0next,zsg,nlvl &
                         ,zsg_full,dzsg_full,zlvls,zfull,compute_zlvls)
                    compute_zlvls=.FALSE.
                 end if
                 ! accumulate sums for next pveght
                 npveght(jtyp)=npveght(jtyp)+1
                 pveghtsum(jtyp)=pveghtsum(jtyp)+medzposz
                 pzmlsum(jtyp)=pzmlsum(jtyp)+0.5*(meto%zmlprev+meto%zmlnext)
                 ! set veghtz based on medzposz, modulo mindveg
                 mindvegz=mindveg
                 if (mindvegz < 9) mindvegz=mindvegz*0.5*(meto%zmlprev+meto%zmlnext)
                 veghtz = max(medzposz,mindvegz)
                 ! compute the specified sveghtz:
                 sveghtz=veght
                 if (sveghtz < 9) sveghtz=sveghtz*0.5*(meto%zmlprev+meto%zmlnext)
                 ! use the smaller of specified and computed veghtz:
                 veghtz = min(veghtz, sveghtz)
              else
                 veghtz = veght
              endif
              CALL PARDSP(METO%UMIX,METO%VMIX,METO%GDISX,METO%GDISY,DT,ZMDL,ZSFC,    &
                   NLVL,METZ(:)%WMIX,ZSG,zsg_full,dzsg_full,xpos(kP),ypos(kp),ZPOS(KP),             &
                   SIGV(KP),SIGW(KP),SIGU(KP),HDWPX,METO%ZNDX,                &
                   VX(job_id+ens_id),                                         &
                   ...
                   rseed, sigmaw, sigmau, samptt, samptt2, wwprev(kp), veghtz,     &
                   convdur(kp), meto%zlocnext, ramsflg,                           &
                   metz(:)%temp, metz(:)%tempprev, metz(:)%rhfr, metz(:)%rhfrprev,            &
                   sphu, foot, meto%muprev, metz(:)%alt0prev, meto%munext, metz(:)%alt0next, vcoord)
              IF(GRID(pgrd(kp),Ktime)%GLOBAL.EQ."gl")THEN
                 IF(XPOS(KP).GE.FLOAT(GRID(pgrd(kp),Ktime)%NX+1)) &
                      XPOS(KP)=XPOS(KP)-FLOAT(GRID(pgrd(kp),Ktime)%NX)
                 IF(XPOS(KP).LT.1.0) XPOS(KP)=GRID(pgrd(kp),Ktime)%NX+XPOS(KP)
                 IF(YPOS(KP).GT.FLOAT(GRID(pgrd(kp),Ktime)%NY)) &
                      YPOS(KP)=2.0*GRID(pgrd(kp),Ktime)%NY-YPOS(KP)
                 IF(YPOS(KP).LT.1.0) YPOS(KP)=2.0-YPOS(KP)
              end IF
              IF(GRID(pgrd(kp),Ktime)%GLOBAL.EQ."nh")THEN
                 IF(XPOS(KP).GE.FLOAT(GRID(pgrd(kp),Ktime)%NX+1)) &
                      XPOS(KP)=XPOS(KP)-FLOAT(GRID(pgrd(kp),Ktime)%NX)
                 IF(XPOS(KP).LT.1.0) XPOS(KP)=GRID(pgrd(kp),Ktime)%NX+XPOS(KP)
                 IF(YPOS(KP).GT.FLOAT(GRID(pgrd(kp),Ktime)%NY)) &
              ...
              IF(GRID(pgrd(kp),Ktime)%GLOBAL.EQ."sh")THEN
                 IF(XPOS(KP).GE.FLOAT(GRID(pgrd(kp),Ktime)%NX+1)) &
                      XPOS(KP)=XPOS(KP)-FLOAT(GRID(pgrd(kp),Ktime)%NX)
                 IF(XPOS(KP).LT.1.0) XPOS(KP)=GRID(pgrd(kp),Ktime)%NX+XPOS(KP)
                 IF(YPOS(KP).LT.1.0) YPOS(KP)=2.0-YPOS(KP)
           ...
           CALL PUFDSP(KPUFF,METO%UMIX,METO%VMIX,DT,ZMDL,ZSFC,NLVL,METZ%WMIX,ZSG, &
                PAGE(KP),ZPOS(KP),SIGH(KP),SIGW(KP),HDWP(KP),METO%ZNDX,vcoord)
           ...
           IF(CDEP) CALL DEPELM(                                                  &
                ...
                HDWPX,METO%RAIN,METZ%DENS,METZ%TEMP,METZ%RHFR,DRYD,vcoord)
           ...
           IF(ICHEM.EQ.4)THEN
              ...
              CALL METSUM(CONC,NUMGRD,XPOS(KP),YPOS(KP),METO%GDISX,METO%GDISY,    &
                   ...
                   SIGH(KP),SIGW(KP),HDWPX,PTYP(KP),CSUM,vcoord)
           ELSE
              IF(CMASS.EQ.0)THEN
              ...
              ELSE
                 ...
                 CALL MASSUM(CONC,NUMGRD,METO%PLAT,METO%PLON,DT,JET,ZMDL,ZSFC,KMSL,  &
                      CGSIZE,MASS(:,KP),ZPOS(KP),PTYP(KP),CSUM,vcoord)
              END IF
           END IF !ichem.eq.4
           ...
           IF(LAGSAM.AND.HDWPX.EQ.6)                                              &
                ...
                ZMDL,ZSFC,KMSL,MASS(:,KP),PAGE(KP),PTYP(KP),CSUM,vcoord)
...
!!$                 ZX=1.0
!!$              END IF
              IF (.NOT.RAMSFLG) CALL ind_zsg (zmdl,zsg,nlvl,sgt,zx,aa,bb,cc,vcoord)
...
!!$                 ZX=1.0
!!$              END IF
              IF (.NOT.RAMSFLG) CALL ind_zsg (zmdl,zsg,nlvl,sgb,zx,aa,bb,cc,vcoord)
           ...
           if (vcoord .eq. 1) then
              ZOUT=(1.0-ZPOS(KP))*(ZMDL-ZSFC)
           elseif (vcoord .eq. 2) then
              call eta2agl(zpos(kp),zout,meto%munext,metz(:)%alt0next,zsg,nlvl &
                   ,zsg_full,dzsg_full,zlvls,zfull,compute_zlvls)
              compute_zlvls=.FALSE.
           end if
           ...
           IF(WRITEOUT)THEN
              ...
              CALL OUTPUT(KFPARDAT,IVMAX,VARSIWANT(1:IVMAX),NTURB,             &
                   ...
                   METO%ZLOCNEXT,PRESLOCAL,TEMPLOCAL,ptyp(kp))
              ! JCL:              reinialize the cumulative SAMPTT
              SAMPTTCUM(KP)=0.0
              FOOTCUM(KP)=0.0
              ZFXCUM(KP)=0.0
              ! Terminate particles older than maximum age, once they have been written out
              if (abs(PAGE(kp)) .ge. maxpage) pgrd(kp)=0

              ! MR 20140115: terminate particles which left the region of interest, once they have been written out
              if (minplon .lt. minplat) then
                 in_latlon_box=xout .ge. minplon .and. xout .le. maxplon
              else
                 ! for lon box straddling the dateline, e.g. minplon=170,maxplon=-170
                 in_latlon_box=xout .ge. minplon .or. xout .le. maxplon
              end if
              in_latlon_box = in_latlon_box .and. yout .ge. minplat .and. yout .le. maxplat
              if (.not. in_latlon_box) pgrd(kp)=0
        ...
        IF(LAGSAM) CALL LAGOUT                                                &
             ...
             (LAGS,JET,KPM,DT,MASS,XPOS,YPOS,ZPOS,HDWP,PAGE,PTYP,PGRD,ZMDL,vcoord)
        ...
        IF(all(COUNTNPAROUT > MAX(INT(OUTFRAC*KPM),1))) GO TO 300
        !*************************************************************************
        !
        !-------------------------------------------------------------------------------
     ...
     call system_clock(clock_cycle2,clock_rate,clock_max)
     elapsed=real(clock_cycle2 - clock_start)/real(clock_rate)
     if (elapsed .lt. 0) elapsed=elapsed+real(clock_max)/real(clock_rate)

     WRITE(*,'(1X,A,F5.1,a,i6,g15.6)')'Percent complete: ',PCNT &
          ,' hour, elapsed=',kh,elapsed
     ...
     IF(NDUMP.GT.0.AND.KH.EQ.NDUMP)THEN
        ...
        CALL PAROUT(JET,KPM,MASS,XPOS,YPOS,ZPOS,SIGH,SIGU,    &
             SIGV,SIGW,HDWP,PAGE,PTYP,PGRD,NSORT,ZMDL,vcoord)
...
300 CONTINUE
  ...
  DEALLOCATE (page,hdwp,ptyp,pgrd,nsort,spot,zsg,zsg_full,dzsg_full,metz)  
  if (vcoord .eq. 2 .and. awrfflg) DEALLOCATE (zlvls,zfull)
  ...
  ! deallocate arrays for dynamic veght estimation
  deallocate(dyn_veg,pveght,pveghtsum,pzmlsum,medzpos,npveght,pdyn_veg)

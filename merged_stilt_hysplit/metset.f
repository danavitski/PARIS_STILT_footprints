...
!-------------------------------------------------------------------------------
  ...
  INTEGER,  PARAMETER   :: mlenh  = 15000  ! maximum length of header
...
! fixed portion (108) of the extended header
  ...
  OPEN(KUNIT, FILE=FNAME,STATUS='OLD',ACTION='READ',                           &
       RECL=recl_k108,ACCESS='DIRECT',FORM='UNFORMATTED')
  READ(KUNIT,REC=1)LABEL(1:k50),HEADER(1:k108)
...
! decode the standard portion of the index record
  ...
  if (k50 .eq. 50) then
     READ(LABEL,'(5I2,2X,A2,A4)')                                   &
          FILE(KG,KT)%FIRST%YR, FILE(KG,KT)%FIRST%MO, FILE(KG,KT)%FIRST%DA,     &
          FILE(KG,KT)%FIRST%HR, FILE(KG,KT)%FIRST%IC, CGRID, KVAR
  else
     READ(LABEL,'(5I2,4X,A2,A4)')                                   &
          FILE(KG,KT)%FIRST%YR, FILE(KG,KT)%FIRST%MO, FILE(KG,KT)%FIRST%DA,     &
          FILE(KG,KT)%FIRST%HR, FILE(KG,KT)%FIRST%IC, CGRID, KVAR
  endif
...
!dwen(20090817)  LDAT = GRID(KG,KT)%NX*GRID(KG,KT)%NY
      ...
      IF (GRID(KG,kt)%MODEL_ID  == 'RAMS'  .OR.  GRID(KG,kt)%MODEL_ID(1:3)  == 'ECX' .or. &
           GRID(KG,kt)%MODEL_ID  == 'DWRF' .or. GRID(KG,kt)%MODEL_ID  == 'NWRF' .or. &
           GRID(KG,kt)%MODEL_ID  == 'DGEO') THEN
...
! read extended character string over multiple index records
  ...
  DO N=1,NNDX
     ...
     READ(KUNIT,REC=KREC)LABEL(1:k50),HEADER(NHL1:NHL2)
     KREC=KREC+1
     NHL1=NHL2+1
  END DO
...
! CHG (10/01/03) change from 8 to 16 bit
            IF(GRID(KG,kt)%MODEL_ID /= 'RAMS' .AND. GRID(KG,kt)%MODEL_ID(1:3) /= 'ECX' .AND. &
                 GRID(KG,kt)%MODEL_ID  /= 'DWRF' .and. GRID(KG,kt)%MODEL_ID  /= 'NWRF' .and. &
                 GRID(KG,kt)%MODEL_ID  /= 'DGEO')THEN
...
! decode the forecast hour and minutes from the extended header
  READ(HEADER(1:k108),'(4X,I3,I2)') FILE(KG,KT)%FIRST%IC, FILE(KG,KT)%FIRST%MN
...
! decode the forecast hour and minutes from the extended header
  READ(HEADER(1:k108),'(4X,I3,I2)') FILE(KG,KT)%LAST%IC, FILE(KG,KT)%LAST%MN
...
!-------------------------------------------------------------------------------
    ...
    eloop : DO WHILE (NREC.GE.1)
       ...
       READ(KUNIT,REC=NREC,IOSTAT=KRET) LABEL(1:k50), HEADER(1:k108)
       ...
       if (k50 .eq. 50) then
          ...
          READ(LABEL,'(5I2,6X,A4)',IOSTAT=KRET)                                   &
               FILE(KG,KT)%LAST%YR, FILE(KG,KT)%LAST%MO, FILE(KG,KT)%LAST%DA,     &
               FILE(KG,KT)%LAST%HR, FILE(KG,KT)%LAST%IC, KVAR
       endif
       IF(KRET.NE.0)EXIT eloop
       IF(KVAR.NE.'INDX') EXIT eloop
       READ(HEADER(1:k108),'(7X,I2)')FILE(KG,KT)%LAST%MN
       NREC=NREC+DREC(KG,KT)%REC_PER

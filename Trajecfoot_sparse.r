Trajecfoot <-function(ident,part=NULL,timelabel=NULL,pathname="",foottimes=c(0,360),zlim=c(0,0),fluxweighting=NULL,coarse=1,dmassTF=T,
                    ...
                    sparse=TRUE, significance = 15, rundir=NULL){
# Creates footprint for individual parrticle runs
# footprints are in units of ppm/(micro-moles/m2/s)
...
# 'significance'    controls the number of significant digits in both dense and sparse formats
...
print(paste("Trajecfoot(): amount of recorded particle locations after filtering out particles that went out of domain at background boundaries =",nrow(part),sep=""))
...
print(paste("Trajecfoot(): amount of recorded particle locations after filtering out particles that had no exchange with surface =",nrow(part),sep=""))
...
# check if part is empty. if so, return function
#if(is.null(dim(part)))return()
if(nrow(part)==0){
   print(paste("Trajecfoot(): no exchange of particles with surface is simulated, moving on...",sep=""))
   return(NULL)
}
...
# Create lat,lon grid for simulation
lons              <- seq(lon.ll+(0.5*lon.res),(lon.ll+0.5*lon.res)+(lon.res*numpix.x),lon.res)
lats              <- seq(lat.ll+(0.5*lat.res),(lat.ll+0.5*lat.res)+(lat.res*numpix.y),lat.res)

# Sparse footprints --> create four empty lists for output: index_x, index_y, footvalue, timelist
...
if (sparse == TRUE) {
   foot_sparse <- create_sparse_matrix_coords(emissgrid.all, lons = lons, lats = lats)
   
   if (sum(foot_sparse$infl) != 0){
      # Give an indication of the order of magnitude of the influence field
      #print(sum(foot_sparse$infl))

      timevar = rep(foottimespos, length(foot_sparse$infl))
      index_x = append(index_x, foot_sparse$lon)
      index_y = append(index_y, foot_sparse$lat)
      valuelist = append(valuelist, foot_sparse$infl)
      timelist = append(timelist, timevar)
      }
...
#count = count + 1
#print(paste("simulation timestep: ", count))
...
if (sparse == TRUE & sum(foot_sparse$infl) != 0) {
   
   #return(list("infl" = valuelist, "times" = timelist, "index_x" = index_x, "index_y" = index_y))
   return(list("infl" = valuelist, "times" = timelist, "lons" = index_x, "lats" = index_y))

} else if (sum(foot_sparse) == 0){
   print(paste("Trajecfoot(): no exchange of particles with surface is simulated, moving on...",sep=""))
   return(NULL)

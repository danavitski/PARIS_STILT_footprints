...
#  day<-day[sel];mon<-mon[sel];yr<-yr[sel];hr<-hr[sel]
  ...
  yr<-paste(substring(10000+abs(yr),2,5),sep="") #4 digits 
  mon<-paste(substring(100+abs(mon),2,3),sep="") #2 digits 
  metfile<-paste("ecmw.",yr,mon,day,".arl",sep="")
  return(metfile)
}#if rams
...
#  day[sel]<-31;mon[sel]<-1
  ...
  # special case June 2013 : ECMWF file for 2013 may is from 2013 may 01 03:00 until june 26. 0:00
  #                          ECMWF file for 2013 jun is from june 26 03:00 until jul 01. 0:00
  if ('1306' %in% yrmns) { # any used day is in june 2013
   yrmns<-c(yrmns,1305) # append always the may 2013 file (which goes until june 26)
   yrmns<-unique(yrmns) # make it again unique
   }

  # special case Feb 2006 : ECMWF file for 2006 January is from jan 01 until feb 02. 0:00
  #                         ECMWF file for 2006 februay is from feb 02 03:00 to march 01. 0:00
  if ('0602' %in% yrmns) { # any used day is in feb 2006
   yrmns<-c(yrmns,0601) # append always 2006 january file (which goes until feb 02 00:00)
   yrmns<-unique(yrmns) # make it again unique
   }
...
#get all days involved
  #ensure to avoid gap between 21 and 0 hours between met files
  if(hr>21&nhrs<0){nhrs<-nhrs-(24-hr);nt<-weekdayhr(yr,mon,day,hr+24-hr,0);day=nt[,"day"];mon=nt[,"mon"];yr=nt[,"yr"];hr=nt[,"hr"]}
  ndays<-floor((abs(nhrs)+24-hr)/24)
  if(yr<50){yr<-yr+2000} else if(yr<100) {yr<-yr+1900}
  newtime<-weekdayhr(yr,mon,day,hr,sign(nhrs)*60*24*(0:ndays))
  day<-newtime[,"day"]; mon<-newtime[,"mon"]; yr<-newtime[,"yr"]; hr<-newtime[,"hr"];
  jdat<-julian(mon,day,yr)
  weeks<-trunc((day-1)/7)+1
  yr<-paste(substring(10000+abs(yr),4,5),sep="") #2 digits 
  monnames<-c("jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec")
  mons<-monnames[mon]
  metfile<-unique(paste("gdas1.",mons,yr,".w",weeks,sep=""))
  return(metfile)
}#if gdas

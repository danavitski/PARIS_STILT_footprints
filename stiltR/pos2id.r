pos2id<-function(jultime,lat,lon,alt,npars=NULL,ens_mem_num=NULL,sep="x"){
...
# Amount of particles
npars<-substring(10000+npars,2)

#put everything together, and include possibly the ensemble member number for multi-ens (sensitivity) runs
if (!is.null(ens_mem_num)){
  ens_mem_num<-substring(100+ens_mem_num,2)
  paste(yr4,mon,day,hr,lat,lon,alt,npars,ens_mem_num,sep=sep)
} else {
  paste(yr4,mon,day,hr,lat,lon,alt,npars,sep=sep)}
}
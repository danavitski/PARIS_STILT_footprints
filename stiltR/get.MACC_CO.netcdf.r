...
get.MACC_CO.netcdf <- function(yr4=NULL, mon=NULL, day=NULL, hr=NULL, co2inifile=NULL,
...
# --------------------------------------------------------------------------------------------------
   ...
   yearmon<-paste(refdate$year,substring(refdate$month+100,2,3),sep="_")
#
   for(yearmoni in unique(yearmon)){
      selm<-yearmon==yearmoni
      resultm<-result[selm,]
      result.selm<-result.sel[selm]
      fn<-basename(co2inifile)
      co2inifilem<-paste(dirname(co2inifile),"/",substring(fn,1,nchar(fn)-10),yearmoni,".nc",sep="")
      ...
      for (time.i in unique(timept)) {
         ...
         if(length(ctsel)==1){
           ...
           if(any(count.rd>1)) tm3boundary[ctsel] <- get.var.ncdf(tm3file, varid="co", start=c(start.rd, time.i),count=c(count.rd,1))[pt3d[,count.rd>1]]
           if(!any(count.rd>1))tm3boundary[ctsel] <- get.var.ncdf(tm3file, varid="co", start=c(start.rd, time.i),count=c(count.rd,1)) #only one value
           #Note that scale factor and ini (global offset) has been applied in get.var.ncdf
         }
      }

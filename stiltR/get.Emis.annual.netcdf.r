get.Emis.annual.netcdf<-function(fdate,i,j,ires,jres,fuel="oil",numpix_x,numpix_y){

#print(lon.ll)
#print(lat.ll )

#function to extract interannual scaling factors for surface fluxes at a specific resolution from maps stored in netcdf files
...
#fuel: "coal" or "gas" or "oil"
#BPpath: path to the netcdf file, set in set.edgar.r
...
fname <- paste(BPpath,tolower(fuel),".nc",tmp,sep="")
...
#---------end old -version tk ------
#---------new version tk ----------
#if(any(fdate>2013)){fdate[fdate>2013]<-2013;print("NOTICE: annual emission data only until 2013; replaced more recent years by 2013")}
res <- .Fortran('getEmisnetCDF',
                as.character(fname), n, as.integer(fdate),

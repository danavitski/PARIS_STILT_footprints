...
shlibpath <- "/Net/Groups/BSY/people/cgerbig/STILT_merged_svn/stiltR/shlib/"   # R extensions
...
###### define receptors ######
Timesname<-".HD.2011" #name of object containing frac. julian, lat, lon, agl (m) as receptor information;
...
emisscatTF<-F #add tracers for different edgar categories/fuel types
if(emisscatTF){
...
#### edit "set.edgar.r" for changing tracers when using different edgar categories/fuel types (i.e. "emisscatTF<-T")
  ...
  c("/User/homes/croeden/public_html/download-CO2-3D/INVERSION/OUTPUT/s96_v3.6_mix_2013.nc",                                     #(not for "climat") full name for initial & boundary cond. file (CO2)
             "",                                                                                          #full name for initial & boundary cond. file (CO)
                      "/Net/Groups/BSY/toolsSTILT/fluxes_input/TM3_CH4/mmix_ch4_s1.b",                         #full name for initial & boundary cond. file (CH4)

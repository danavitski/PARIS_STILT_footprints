...
Trajecmod <- function(partarg=NULL, totpartarg=NULL, nodeoffset=NULL, csv=TRUE) 
{
   #---------------------------------------------------------------------------------------------------
   ...
   #     In this form, Trajecfoot only updates 'foot'. Internally it reads the wrfinput-file
         ...
         print(paste("dimensions of footprint = ",dim(foot),sep=""))
...
##     # Construct longx,latx from info supplied in setStiltparam.r
         ...
         # Check the sum of each simulated footprint
         sumfilename      <- paste(path, 'sum_footprint_', station, '_', identname, '_orig', ".csv", sep="")
         summat = c()

imagell<-function(mat,lg=F,zlims=NULL,zoom=NULL,center=c(-72.172,42.536),rays=F,
                  ...
                  bordersTF=FALSE,part=NULL,locations=NULL,dev="X11",type="",res.png=72,map.title=NULL,cex.part=0.2){
...
#'bordersTF' if T, plot country boundaries
#'part' a particle location object can be passed on, it will be plotted on top of footprints
#'locations' an object with locations to plot, with column names "lat" and "lon", and rownames that are used as labels on the plot
...
if(dev=="X11"&bordersTF)x11(type="dbcairo")
  if(lg)mat<-log10(mat)
...
#  }#if bordersTF
  ...
  if(!is.null(part)){ #plot particle location on top
    ...
    if("foot"%in%dimnames(part)[[2]]){
      points(part[part[,"foot"]==0,c("lon","lat")],cex=cex.part,pch=16,col="gray")
      points(part[part[,"foot"]>0,c("lon","lat")],cex=cex.part,pch=16,col="black")
    } else {
      points(part[,c("lon","lat")],cex=cex.part,pch=16,col="black")
    }
  }
 
  if(!is.null(locations)){ #plot locations on top
    points(locations[,c("lon","lat")],cex=1,pch=16,col="black")
    text(coords[,c(2,1)],labels<-rownames(coords),adj=c(0.5,1.2),cex=1.5) #labels underneath
  }

  if(type=="foot")zlab<-expression(paste("surface influence [ppm/(",mu,"mole/(",m^2*s,"))]"))
  if(type=="infl")zlab<-expression(paste("influence [1/",m^2,"]"))
  if(type=="flux")zlab<-expression(paste("flux [",mu,"mole/(",m^2*s,")]"))
  if(type=="emis")zlab<-expression(paste("emission [",mu,"mole/(",m^2*s,")]"))
  if(!(type%in%c("infl","foot","flux","emis")))zlab<-type
